...
class SQLRuntimeSafetyManager:
    ...
    def __init__(
        ...
        self.logger.info(
            f"SQL Runtime Safety Manager initialized with {safety_level.value} safety level"
        )
        ...
        try:
            ...
            for db_name, active_count in self._active_connections.items():
                if active_count > self.max_connections_per_db * 1.5:
                    self.logger.error(
                        f"Connection leak detected for {db_name}: {active_count} active"
                    )
    ...
    def get_safe_connection(
        self, db_path: str, db_type: str = "duckdb"
        ...
        try:
            if db_type == "duckdb":
                ...
                conn = psycopg2.connect(
                    db_path, connect_timeout=30, cursor_factory=psycopg2.extras.RealDictCursor
                )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if result.success:
                    ...
                    self.execution_metrics["avg_execution_time_ms"] = (
                        total_time / self.execution_metrics["successful_queries"]
                    )
                    ...
                    if attempt < max_retries:
                        ...
                        self.logger.warning(
                            f"Query attempt {attempt + 1} failed, retrying in {delay}s"
                        )
                        time.sleep(delay)
                    else:
    ...
    def _cleanup_stale_connections(self, db_name: str):
        ...
        if db_name in self._connection_pools:
            with self._pool_locks[db_name]:
                ...
                self.logger.info(
                    f"Cleaned up connections for {db_name}: {len(pool) - len(healthy_connections)} removed"
                )
...
if __name__ == "__main__":
    ...
    result = safety_manager.execute_safe_query(
        db_path=db_path, query="CREATE TABLE test (id INTEGER, name TEXT)", db_type="duckdb"
    )
