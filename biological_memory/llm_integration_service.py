    ...
    def _get_cached_response(self, prompt_hash: str) -> Optional[LLMResponse]:
        ...
        try:
            with duckdb.connect(self.cache_db_path) as conn:
                ...
                """, [prompt_hash, self.model_name]).fetchone()
        ...
        try:
            with duckdb.connect(self.cache_db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, [
                    prompt_hash, self.model_name, prompt, response.content,
...
def llm_generate(prompt: str, model: str = "gpt-oss", endpoint: str = "", timeout: int = 300) -> str:
    """
...
def llm_generate_json(prompt: str, model: str = "gpt-oss", endpoint: str = "", timeout: int = 300) -> str:
    """
