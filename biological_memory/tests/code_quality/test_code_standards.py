...
class TestCodeStandards(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.excluded_dirs = {
            'target', 'dbt_packages', 'logs', '__pycache__', '.git',
            'node_modules', 'venv', '.venv', 'packages'
        }
    ...
    def test_python_syntax_validity(self):
        ...
        if violations:
            ...
            for violation in violations:
                violation_msg += f"  {violation['file']}:{violation.get('line', '?')} - {violation['error']}\n"
    ...
    def test_sql_basic_syntax(self):
        ...
        if violations:
            ...
            for violation in violations:
                violation_msg += f"  {violation['file']}:{violation.get('line', '?')} - {violation['issue']}\n"
    ...
    def test_documentation_standards(self):
        ...
        critical_files = ['error_handling.py', 'orchestrate_biological_memory.py', 'llm_integration_service.py']
        critical_violations = [v for v in violations if any(cf in v['file'] for cf in critical_files)]
    ...
    def test_macro_naming_conventions(self):
        ...
        if violations:
            ...
            for violation in violations:
                violation_msg += f"  {violation['file']} - {violation['macro']}: {violation['issue']}\n"
            self.fail(violation_msg)
