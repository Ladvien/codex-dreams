...
class LoadScenarioTests(unittest.TestCase):
        ...
        for batch_start in range(0, record_count, batch_size):
            ...
            for i in range(batch_start, min(batch_start + batch_size, record_count)):
                memory_time = datetime.now() - timedelta(
                    days=i % 30,  # Spread across 30 days
                    hours=i % 24,
                    minutes=(i * 13) % 60
                )
    ...
    def test_memory_usage_under_load(self):
        ...
        self.assertLess(total_duration, 1000, f"Load test duration too high: {total_duration:.2f}ms")
    ...
    def tearDownClass(cls):
        ...
        # Clean up test data
        deleted_count = cls.conn.execute("DELETE FROM raw_memories WHERE memory_id LIKE 'load_test_%'").fetchone()
