...
class TestBiologicalMemoryPerformance:
    ...
    def test_working_memory_performance(self, benchmarker, db_connection):
        ...
        assert (
            result["avg_time_ms"] <= 50.0
        ), f"Working memory average time {result['avg_time_ms']:.2f}ms exceeds 50ms target"
        assert (
            result["p95_time_ms"] <= 100.0
        ), f"Working memory P95 time {result['p95_time_ms']:.2f}ms exceeds 100ms limit"
    ...
    def test_semantic_similarity_performance(self, benchmarker, db_connection):
        ...
        ), f"Semantic similarity query failed: {result.get('error')}"
        assert (
            result["avg_time_ms"] <= 200.0
        ), f"Semantic similarity average time {result['avg_time_ms']:.2f}ms exceeds 200ms limit"
    ...
    def test_memory_consolidation_performance(self, benchmarker, db_connection):
        ...
        assert (
            result["status"] == "SUCCESS"
        ), f"Memory consolidation query failed: {result.get('error')}"
        assert (
            result["avg_time_ms"] <= 150.0
        ), f"Memory consolidation average time {result['avg_time_ms']:.2f}ms exceeds 150ms limit"
    ...
    def test_analytics_dashboard_performance(self, benchmarker, db_connection):
        ...
        assert (
            result["status"] == "SUCCESS"
        ), f"Analytics dashboard query failed: {result.get('error')}"
        assert (
            result["avg_time_ms"] <= 50.0
        ), f"Analytics dashboard average time {result['avg_time_ms']:.2f}ms exceeds 50ms target"
    ...
    def test_llm_cache_performance(self, benchmarker, db_connection):
        ...
        result = benchmarker.benchmark_query(db_connection, query, "llm_cache_lookup", "caching")

        assert (
            result["status"] == "SUCCESS"
        ), f"LLM cache lookup query failed: {result.get('error')}"
        assert (
            result["avg_time_ms"] <= 25.0
        ), f"LLM cache lookup average time {result['avg_time_ms']:.2f}ms exceeds 25ms target"
    ...
    def test_comprehensive_performance_regression(self, benchmarker, db_connection):
        ...
        critical_queries = [
            ...
            (
                "SELECT * FROM raw_memories ORDER BY created_at DESC LIMIT 10",
                "recent_memories",
                "basic",
            ),
            (
                "SELECT memory_type, COUNT(*) FROM raw_memories GROUP BY memory_type",
                "memory_type_summary",
                "analytics",
            ),
        ]
        ...
        for query, name, query_type in critical_queries:
            result = benchmarker.benchmark_query(
                db_connection, query, name, query_type, iterations=20
            )
        ...
        assert len(successful_queries) == len(
            all_benchmarks
        ), "Some queries failed during regression testing"
        assert (
            avg_system_performance <= 50.0
        ), f"System average performance {avg_system_performance:.2f}ms exceeds 50ms target"
    ...
    def test_memory_intensive_operations(self, benchmarker, db_connection):
        ...
        assert (
            result["status"] == "SUCCESS"
        ), f"Memory intensive query failed: {result.get('error')}"
        assert (
            result["avg_time_ms"] <= 300.0
        ), f"Memory intensive average time {result['avg_time_ms']:.2f}ms exceeds 300ms limit"
