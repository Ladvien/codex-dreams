...
class SimplePerformanceTests(unittest.TestCase):
    ...
    def test_working_memory_performance(self):
        ...
        print(f"  📊 Working Memory Query: {avg_time:.2f}ms (target: {self.targets['working_memory_ms']}ms)")
        
        self.assertLess(avg_time, self.targets['working_memory_ms'], 
    ...
    def test_memory_consolidation_performance(self):
        ...
        avg_time = self._time_query(query)
        print(f"  📊 Consolidation Batch: {avg_time:.2f}ms (target: {self.targets['consolidation_ms']}ms)")
        
        self.assertLess(avg_time, self.targets['consolidation_ms'],
    ...
    def test_partitioned_query_performance(self):
        ...
        avg_time = self._time_query(query)
        print(f"  📊 Partitioned Query: {avg_time:.2f}ms (target: {self.targets['partitioned_query_ms']}ms)")
        
        self.assertLess(avg_time, self.targets['partitioned_query_ms'],
