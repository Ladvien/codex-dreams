...
class PostgresDataFlowTests(unittest.TestCase):
    ...
    def test_postgres_connection_health(self):
        ...
        print(
            f"Testing connection to: {masked_config['host']}:{masked_config['port']}/{masked_config['database']}"
        )

        result = self.duck_conn.execute(
            "SELECT COUNT(*) as count FROM codex_db.public.memories"
        ).fetchone()
    ...
    def test_data_freshness(self):
        ...
        if latest_memory:
            hours_since_latest = (
                datetime.now() - latest_memory.replace(tzinfo=None)
            ).total_seconds() / 3600
            self.assertLess(
                hours_since_latest, 168, "Latest data should be within a week"
            )  # 7 days

        print(
            f"✅ Data freshness test passed: Latest={latest_memory}, Recent={recent_count}, Total={total_count}"
        )
    ...
    def test_data_transformation_accuracy(self):
        ...
        print(
            f"✅ Transformation test passed: {record_count} records, {memory_type_count} types, avg activation={avg_activation:.3f}"
        )
    ...
    def test_performance_benchmarks(self):
        ...
        self.assertLess(
            query_time_2, 15.0, "Complex transformation should complete within 15 seconds"
        )
        self.assertGreater(len(result), 0, "Should return memory type classifications")

        print(
            f"✅ Performance test passed: Simple query={query_time_1:.2f}s, Complex query={query_time_2:.2f}s"
        )
    ...
    def test_error_handling_resilience(self):
        ...
        print(
            f"✅ Error handling test passed: {total_records} total, {null_content} null, {empty_content} empty"
        )
    ...
    def test_data_consistency_validation(self):
        ...
        print(
            f"✅ Data consistency test passed: {unique_ids} unique IDs, {valid_temporal} valid temporal records"
        )
    ...
    def test_integration_end_to_end(self):
        ...
        print(
            f"✅ End-to-end test passed: {processed_count} records processed, {memory_types} types, {creation_days} days of data"
        )
