...
class TestLLMIntegrationService(unittest.TestCase):
    ...
    def test_cache_functionality(self):
        ...
        service = OllamaLLMService(ollama_url=self.ollama_url, cache_db_path=self.cache_db_path)
    ...
    def test_ollama_api_call_success(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"response": '{"goal": "Test Goal"}', "eval_count": 50}
    ...
    def test_health_check_healthy(self, mock_get):
        ...
        service = OllamaLLMService(
            ollama_url=self.ollama_url, model_name="gpt-oss:20b", cache_db_path=self.cache_db_path
        )
...
class TestLLMIntegrationEndToEnd(unittest.TestCase):
    ...
    def test_end_to_end_llm_workflow(self):
        ...
        service = initialize_llm_service(
            ollama_url="http://localhost:11434", model_name="gpt-oss:20b"
        )
...
class TestLLMIntegrationPerformance(unittest.TestCase):
    ...
    def test_concurrent_requests_performance(self, mock_post):
        ...
        # Mock fast response
        mock_response = Mock()
        mock_response.json.return_value = {"response": '{"goal": "Test Goal"}', "eval_count": 10}
        mock_response.raise_for_status.return_value = None
        mock_post.return_value = mock_response

        service = OllamaLLMService(
            ollama_url="http://127.0.0.1:11434", cache_db_path=self.cache_db_path
        )
