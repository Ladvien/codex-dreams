...
class TestMemoryHealthAnalytics:
    ...
    def _insert_sample_data(self, conn: duckdb.DuckDBPyConnection):
        ...
        working_memories = [
            ("wm_001", "Current project presentation prep", ["presentation", "project"], 0.85, base_time - timedelta(minutes=5)),
            ("wm_002", "Budget review meeting notes", ["budget", "meeting"], 0.75, base_time - timedelta(minutes=10)),
            ("wm_003", "Client email response draft", ["client", "email"], 0.70, base_time - timedelta(minutes=15)),
            ("wm_004", "Team schedule coordination", ["schedule", "team"], 0.65, base_time - timedelta(minutes=20)),
            ("wm_005", "Coffee machine repair status", ["coffee", "repair"], 0.60, base_time - timedelta(minutes=25)),
            ("wm_006", "Supply order requirements", ["supplies", "order"], 0.55, base_time - timedelta(minutes=30)),
            ("wm_007", "Document version control", ["document", "version"], 0.50, base_time - timedelta(minutes=35))
        ]
        ...
        for i, (mem_id, content, concepts, strength, created) in enumerate(working_memories):
            ...
            """, [
                ...
                0.8 - i*0.1, 0.3 + i*0.05, i+1, 0.4 + i*0.1, base_time
            ])
        
        # Short-term memory data (various consolidation states)
        stm_memories = [
            ("stm_001", "Project launch strategy discussion", "Project Management and Execution", 0.75, 3, True, base_time - timedelta(hours=2)),
            ("stm_002", "Client feedback on proposal", "Client Relations and Service", 0.65, 2, False, base_time - timedelta(hours=4)),
            ("stm_003", "Financial quarter analysis", "Financial Planning and Management", 0.80, 4, True, base_time - timedelta(hours=1)),
            ("stm_004", "Team meeting coordination", "Communication and Collaboration", 0.55, 1, False, base_time - timedelta(hours=6)),
            ("stm_005", "System maintenance checklist", "Operations and Maintenance", 0.70, 3, True, base_time - timedelta(hours=3))
        ]
        ...
        for mem_id, content, goal, strength, coactivation, ready, timestamp in stm_memories:
            ...
            """, [
                mem_id, content, timestamp, json.dumps({}), goal, 
                json.dumps(["task1", "task2"]), json.dumps(["action1"]), json.dumps([]),
                json.dumps({}), strength, coactivation, ready, 0.7, 0.8, 0.6, 
                coactivation, base_time
            ])
        
        # Consolidated memory data (various consolidation fates)
        consolidated_memories = [
            ("con_001", "Strategic planning framework", "Strategy", "executive_function", "prefrontal_cortex", 0.85, "cortical_transfer"),
            ("con_002", "Client relationship protocols", "Communication", "social_cognition", "temporal_superior_cortex", 0.75, "hippocampal_retention"),
            ("con_003", "Budget allocation methodology", "Financial", "quantitative_reasoning", "parietal_cortex", 0.90, "cortical_transfer"),
            ("con_004", "Task coordination patterns", "Project", "temporal_sequencing", "frontal_motor_cortex", 0.65, "gradual_forgetting"),
            ("con_005", "Maintenance procedures", "Operations", "technical_procedures", "motor_cortex", 0.40, "rapid_forgetting")
        ]
        
        for i, (mem_id, content, goal, category, region, strength, fate) in enumerate(consolidated_memories):
            conn.execute("""
                INSERT INTO memory_replay VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'consolidated', ?)
            """, [
                mem_id, content, goal, json.dumps([]), json.dumps([]), json.dumps([]),
                f"{content} gist", category, region, strength, json.dumps({}),
                strength * 0.9, strength * 0.8, strength * 0.7, 0.6, 0.1, fate,
                0.7, 0.6, 3, base_time - timedelta(hours=i*2), 1
            ])
        ...
        for i, (mem_id, content, stability, associations, importance) in enumerate(ltm_memories):
            ...
            """, [
                mem_id, content, ["concept1", "concept2"], 0.8, created_time, created_time,
                5+i, 0.7, 0.6, 0.5, 0.7, created_time, associations, 0.6, 0.8,
                0.5, 0.4, 0.1, stability, importance, 0.7, base_time
            ])
    ...
    def test_memory_distribution_calculations(self, test_db):
        ...
        assert result[2] == 3, f"Expected 3 consolidating memories (cortical_transfer + hippocampal_retention), got {result[2]}"
    ...
    def test_memory_age_analysis(self, test_db):
        ...
        assert result[0] >= 7, f"Should have at least 7 recent memories (working memory), got {result[0]}"
    ...
    def test_biological_rhythm_indicators(self, test_db):
        ...
        assert result[1] in ['wake_hours', 'deep_consolidation_window', 'sleep_hours'], f"Invalid circadian phase: {result[1]}"
        assert result[2] in ['active_processing', 'memory_consolidation', 'rest_recovery'], f"Invalid optimal activity: {result[2]}"
    ...
    def test_data_consistency_and_integrity(self, test_db):
        ...
        assert result[1] >= 0, f"Consolidation processed count should be non-negative, got {result[1]}"
        assert all(val >= 0 for val in result[2:5]), f"Average strengths should be non-negative: {result[2:5]}"
        assert all(val == 0 for val in result[5:8]), f"Should have no negative values: {result[5:8]}"
