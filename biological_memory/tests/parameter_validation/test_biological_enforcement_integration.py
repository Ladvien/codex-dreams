...
class TestBiologicalParameterEnforcement:
    ...
    def test_millers_law_comprehensive_enforcement(self):
        ...
        assert (
            "memory_rank <=" in wm_content or "memory_rank, 1) <=" in wm_content
    ...
    def test_five_second_refresh_cycle_validation(self):
        ...
        if orchestration_path.exists():
            with open(orchestration_path, "r") as f:
                content = f.read()
            assert (
                "sleep(5)" in content or "time.sleep(5)" in content
        ...
        if crontab_path.exists():
            with open(crontab_path, "r") as f:
                content = f.read()
            assert "sleep 5" in content, "Crontab must implement 5-second working memory cycles"

        # Check timing documentation exists
        timing_doc_path = self.project_dir / "TIMING_PATTERNS_DOCUMENTATION.md"
        assert (
            timing_doc_path.exists()
    ...
    def test_memory_consolidation_patterns(self):
        ...
        assert (
            consolidation_threshold < ltm_threshold
        ), "Consolidation threshold should be lower than LTM threshold"
    ...
    def test_biological_timing_hierarchy(self):
        ...
        assert (
            recent_window < weekly_window < monthly_window
    ...
    def test_comprehensive_biological_test_suite(self):
        ...
        comprehensive_test = (
            ...
            / "test_biological_accuracy_comprehensive.sql"
        )
    ...
    def test_timing_pattern_validation_suite(self):
        ...
        timing_test = (
            ...
            / "tests"
            / "parameter_validation"
            / "test_timing_pattern_validation.sql"
        )
...
class TestBiologicalAccuracyValidation:
    ...
    def test_can_run_biological_validation_tests(self):
        ...
        if test_file.exists():
            ...
            assert (
                "CASE" in content.upper() and "WHEN" in content.upper()
            ), "Test should have conditional validation logic"
    ...
    def test_biological_constraints_enforced(self):
        ...
        if wm_path.exists():
            ...
            assert (
                "{{ var('working_memory_capacity') }}" in content
            ), "Working memory must enforce configurable capacity"

        # Check STM model uses proper duration
        stm_path = (
            self.project_dir / "models" / "short_term_memory" / "stm_hierarchical_episodes.sql"
        )
        if stm_path.exists():
...
if __name__ == "__main__":
    ...
    exit_code = pytest.main(
        [__file__, "-v", "--tb=short", "-x"]  # Stop on first failure for faster feedback
    )
