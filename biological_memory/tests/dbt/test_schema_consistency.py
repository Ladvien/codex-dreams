            ...
            with open(compiled_file) as f:
                ...
                for source_table in source_tables:
                    ...
                    if wrong_ref in content:
                        issues_found.append(
                            f"ERROR: Source table {source_table} referenced as 'main' in {compiled_file}"
                        )
                ...
                for model_table in model_tables:
                    ...
                    if wrong_ref in content:
                        issues_found.append(
                            f"ERROR: Model table {model_table} referenced as 'public' in {compiled_file}"
                        )
...
def test_cross_schema_joins():
    ...
    expected_patterns = [
        ...
        (
            'FROM "memory"."public"."memory_similarities"',
            'WHERE target_memory_id IN (SELECT memory_id FROM "memory"."main"',
        ),
    ]
