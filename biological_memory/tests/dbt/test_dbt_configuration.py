...
class TestDbtConfiguration:
    ...
    def test_dbt_project_yml_valid(self):
        ...
        required_vars = [
            'working_memory_capacity', 'short_term_memory_duration',
            'long_term_memory_threshold', 'hebbian_learning_rate',
            'synaptic_decay_rate', 'homeostasis_target'
        ]
    ...
    def test_models_directory_structure(self):
        ...
        expected_subdirs = [
            ...
            "long_term_memory", "semantic", "analytics"
        ]
    ...
    def test_biological_parameters_valid(self):
        ...
        assert 5 <= vars_config['working_memory_capacity'] <= 9, "Working memory capacity out of biological range"
        assert vars_config['short_term_memory_duration'] > 0, "Short-term memory duration must be positive"
...
class TestDbtMacros:
    ...
    def test_macro_compilation(self):
        ...
        for macro_file in macros_dir.glob("*.sql"):
            ...
            assert content.count('{%') == content.count('%}'), f"Unmatched Jinja blocks in {macro_file}"
            assert content.count('{{') == content.count('}}'), f"Unmatched Jinja expressions in {macro_file}"
    ...
    def test_model_materialization_config(self):
        ...
        for model_file in working_memory_models:
            with open(model_file, 'r') as f:
                content = f.read()
            assert "materialized='view'" in content, f"Working memory model {model_file.name} should be view"
        ...
        for model_file in models_dir.rglob("*.sql"):
            ...
            if "FROM " in content or "JOIN " in content:
                ...
                if "biological_memory" in content:
                    assert "ref(" in content or "source(" in content, f"Model {model_file.name} missing ref/source"
