...
class TestStoryDB003Configuration(unittest.TestCase):
    ...
    def test_profiles_yml_exists_and_valid(self):
        ...
        self.assertTrue(
            self.profiles_path.exists(), f"profiles.yml not found at {self.profiles_path}"
        )
    ...
    def test_duckdb_extensions_configuration(self):
        ...
        for target in ["dev", "prod", "test"]:
            ...
            for ext in required_extensions:
                self.assertIn(
                    ext, extensions, f"Missing required extension '{ext}' in target '{target}'"
                )
    ...
    def test_ollama_connectivity(self):
        ...
        try:
            ...
            self.assertTrue(
                gpt_oss_available, f"gpt-oss model not found in available models: {models}"
            )
