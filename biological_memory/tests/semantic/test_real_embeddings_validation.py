...
from llm_integration_service import get_llm_service, initialize_llm_service
...
class TestRealEmbeddingsValidation:
    ...
    def test_semantic_similarity_accuracy(self, duckdb_connection, test_concepts):
        ...
        for (c1, c2), sim in similarities.items():
            ...
            elif (c1 in ai_concepts and c2 in non_ai_concepts) or (
                c1 in non_ai_concepts and c2 in ai_concepts
            ):
        ...
        if ai_ai_similarities and ai_nonai_similarities:
            ...
            assert (
                avg_ai_ai > avg_ai_nonai
            ), f"AI concepts should be more similar to each other (AI-AI: {avg_ai_ai:.3f}, AI-nonAI: {avg_ai_nonai:.3f})"
    ...
    def test_md5_vs_real_embeddings_comparison(self, duckdb_connection):
        ...
        assert (
            abs(md5_avg) < 0.3
        ), f"MD5 similarities should be near random (close to 0): {md5_avg:.3f}"
    ...
    def test_embedding_performance(self, duckdb_connection):
        ...
        assert (
            generation_time < 5.0
        ), f"Embedding generation took {generation_time:.2f}s (should be under 5s)"
    ...
    def test_embedding_consistency(self, duckdb_connection):
        ...
        for i in range(1, len(embeddings)):
            ...
            assert (
                similarity > 0.99
            ), f"Embeddings should be consistent across calls: {similarity:.4f}"
