...
class TestPostgresScannerValidation(unittest.TestCase):
    ...
    def test_profiles_yml_example_exists(self):
        ...
        self.assertIn('postgres_scanner', content, "postgres_scanner not found in profiles.yml.example")
        
        # Check for required sections
        self.assertIn('extensions:', content, "extensions section not found in profiles.yml.example")
        self.assertIn('external_sources:', content, "external_sources section not found in profiles.yml.example")
        self.assertIn('postgresql:', content, "postgresql configuration not found in profiles.yml.example")
    
    def test_setup_duckdb_sql_exists(self):
        """Test that setup_duckdb.sql file exists and contains proper postgres_scanner setup."""
        self.assertTrue(
            self.setup_sql_path.exists(), 
            f"setup_duckdb.sql not found at {self.setup_sql_path}"
        )
        
        content = self.setup_sql_path.read_text()
        
        # Check for postgres_scanner installation and loading
        self.assertIn('INSTALL postgres_scanner;', content, "INSTALL postgres_scanner command not found")
    ...
    def test_extension_functionality_basic(self):
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            self.assertTrue(test_passed, "postgres_scanner extension basic functionality validation failed")
    ...
    def test_configuration_completeness(self):
        ...
        for section in required_profiles_sections:
            self.assertIn(section, profiles_content, 
        ...
        for command in required_setup_commands:
            self.assertIn(command, setup_content,
    ...
    def test_readme_postgres_scanner_documentation(self):
        ...
        if readme_path.exists():
            ...
            self.assertIn('postgres_scanner', content, 
                        "postgres_scanner not documented in README.md")
                ...
                if 'extensions:' in line.lower() or 'extension' in line.lower():
                    if "'postgres'" in line or '"postgres"' in line:
                        if 'postgres://' not in line:  # Skip connection strings
                            self.fail(f"Found incorrect postgres extension reference in README.md line {i+1}: {line}")
