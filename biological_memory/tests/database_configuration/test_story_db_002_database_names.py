...
class TestDatabaseNameConfiguration(unittest.TestCase):
    ...
    def test_no_codex_db_references(self):
        ...
        self.assertEqual(len(codex_db_files), 0, 
                        f"Found 'codex_db' references in: {codex_db_files}")
    ...
    def test_model_source_references(self):
        ...
        self.assertEqual(len(incorrect_sources), 0,
                        f"Found incorrect source references: {incorrect_sources}")
...
if __name__ == '__main__':
    ...
    print(f"Database Configuration Success Rate: {((result.testsRun - len(result.failures) - len(result.errors)) / result.testsRun * 100):.1f}%")
