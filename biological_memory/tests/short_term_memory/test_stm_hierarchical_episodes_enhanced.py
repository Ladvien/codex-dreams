...
class TestSTMHierarchicalEpisodesEnhanced:
    ...
    def setup_episode_clustering_test_data(self, conn):
        ...
        episodes = [
            ...
            (
                "ep1_001",
                "Preparing presentation slides for client meeting",
                ["presentation", "client"],
                0.8,
            ...
            (
                "ep1_002",
                "Reviewing client requirements and objectives",
                ["presentation", "client"],
                0.7,
            ...
            (
                "ep1_003",
                "Setting up meeting room and testing projector",
                ["presentation", "client"],
                0.6,
            ...
            (
                "ep2_001",
                "Analyzing quarterly budget reports",
                ["budget", "financial"],
                0.7,
            ...
            (
                "ep2_002",
                "Creating budget variance analysis document",
                ["budget", "financial"],
                0.6,
                base_time - timedelta(minutes=20),
            ...
            (
                "ep3_001",
                "Checking coffee machine repair status",
        ...
        for i, (mem_id, content, concepts, strength, created, accessed, count) in enumerate(
            episodes
        ):
            conn.execute(
                """
    ...
    def test_millers_law_capacity_constraint(self, test_db):
        ...
        assert (
            capacity_counts.get("core_capacity", 0) <= 5
        ), "Core capacity should not exceed 5 (7-2)"
        total_standard = capacity_counts.get("core_capacity", 0) + capacity_counts.get(
            "standard_capacity", 0
        )
        assert total_standard <= 7, "Standard capacity should not exceed 7"

        print(
            f"✅ Miller's Law Test: Selected {len(result)} memories with capacity distribution: {capacity_counts}"
        )
    ...
    def test_episode_clustering_and_temporal_organization(self, test_db):
        ...
        assert (
            pres_cluster["coherence"] == "high_coherence"
        ), "Presentation cluster should be high coherence"
        ...
        assert (
            budget_cluster["coherence"] == "medium_coherence"
        ), "Budget cluster should be medium coherence"
    ...
    def test_spatial_temporal_binding(self, test_db):
        ...
        spatial_memories = [
            (
                "spatial_001",
                "Presenting quarterly results in main conference room",
                ["presentation", "meeting"],
                0.8,
            ),
            (
                "spatial_002",
                "Working on budget analysis from home office",
                ["budget", "remote"],
                0.7,
            ),
            (
                "spatial_003",
                "Client call via video conference platform",
                ["client", "virtual"],
                0.6,
            ),
        ]
        ...
        for i, (mem_id, content, concepts, strength) in enumerate(spatial_memories):
            ...
            """,
                [
                    ...
                    3,
                    "working_memory",
                    1800 + i * 600,  # age_seconds
                    strength * 0.9,  # recency_score
                    strength * 0.8,  # frequency_score
                    i + 1,
                    strength + 0.1,  # memory_rank, hebbian_strength
                    base_time,
                ],
            )
        ...
        ), f"Expected {expected_contexts}, got {actual_contexts}"

        print(
            f"✅ Spatial-Temporal Binding Test: Successfully bound {len(result)} memories with contexts: {actual_contexts}"
        )
    ...
    def test_memory_interference_and_competition(self, test_db):
        ...
        competing_memories = [
            ...
            (
                "comp_003",
                "Presentation slides for annual review meeting",
                ["presentation", "annual"],
                0.7,
            ),
        ...
        for i, (mem_id, content, concepts, strength) in enumerate(competing_memories):
            ...
            """,
                [
                    ...
                    4 - i,
                    "working_memory",
                    3600 - i * 300,  # access_count, age_seconds
                    strength * 0.9,
                    strength * 0.8,  # recency_score, frequency_score
                    i + 1,
                    strength + 0.1,  # memory_rank, hebbian_strength
                    base_time,
                ],
        ...
        assert (
            0.95 <= presentation_competition_sum <= 1.05
        ), f"Competition scores should sum to ~1.0, got {presentation_competition_sum}"
        ...
        for row in result:
            ...
            if row[3] > 0 or row[4] > 0:  # If there's interference
                ...
                ), "Interference should reduce or maintain strength"

        print(
            f"✅ Memory Interference Test: Processed {len(result)} competing memories with interference effects"
        )
        for row in result:
    ...
    def test_consolidation_readiness_with_episodes(self, test_db):
        ...
        if len([r for r in result if r[2] == "high_coherence"]) > 0:
            assert (
                len(high_coherence_ready) > 0
            ), "High coherence episodes should be ready for consolidation"

        print(
            f"✅ Consolidation Readiness Test: {len(ready_memories)}/{len(result)} memories ready for consolidation"
        )
    ...
    def test_episodic_memory_quality_metrics(self, test_db):
        ...
        quality_test_memories = [
            (
                "qual_001",
                "Detailed client presentation in boardroom with full team",
                ["presentation", "client"],
                0.9,
                3,
                True,
            ...
            (
                "qual_002",
                "Quick email check and response",
                ["email", "communication"],
                0.4,
                1,
                False,
            ...
            (
                "qual_003",
                "Comprehensive budget review meeting with department heads",
                ["budget", "meeting"],
                0.8,
                4,
                True,
        ...
        base_time = datetime.now()
        for i, (mem_id, content, concepts, strength, access_count, has_spatial) in enumerate(
            quality_test_memories
        ):
            test_db.execute(
                """
        ...
        if len(high_fidelity) > 0:
            for memory in high_fidelity:
                assert (
                    memory[2] == "high_coherence"
                ), "High fidelity memories should have high coherence"
