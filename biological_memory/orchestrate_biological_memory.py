...
from llm_integration_service import get_llm_service, initialize_llm_service, register_llm_functions

# Import SQL runtime safety system - BMP-HIGH-006
from sql_runtime_safety import (
    SQLRuntimeSafetyLevel,
    SQLRuntimeSafetyManager,
...
class BiologicalMemoryOrchestrator:
    ...
    def __init__(self, base_path: str = None, log_dir: Optional[str] = None):
        ...
        self.circuit_breaker_enabled = (
            os.getenv("MCP_CIRCUIT_BREAKER_ENABLED", "true").lower() == "true"
        )
    ...
    def _init_llm_service(self):
        ...
        try:
            ...
            llm_cache_path = os.getenv(
                "LLM_CACHE_PATH", str(self.base_path / "dbs" / "llm_cache.duckdb")
            )
            ...
            with duckdb.connect(str(db_path)) as conn:
                ...
                if success:
                    ...
                    if health["status"] == "healthy":
                        ...
                        self.logger.warning(
                            f"LLM service unhealthy: {health.get('error', 'Unknown error')}"
                        )
                else:
    ...
    def run_dbt_command(self, command: str, log_file: str, timeout: int = 300) -> bool:
        ...
        if not self._validate_dbt_command(command):
            self.logger.error(
                f"Security: Rejected potentially malicious command: {self._sanitize_command_for_logging(command)}"
            )
            return False
        ...
        def execute_dbt():
            ...
            effective_timeout = (
                min(timeout, self.ollama_timeout_seconds) if "llm" in command else timeout
            )
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.info(
                    f"dbt command completed successfully in {duration:.2f}s: {command}"
                )
                return True
            else:
                ...
                if any(
                    critical in command
                    for critical in ["consolidation", "long_term", "homeostasis"]
                ):
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            overall_healthy = (
                health_results["database_accessible"]
                and len(table_names) > 0
                and health_results["system_resources"].get("memory_percent", 100) < 95
                and health_results["system_resources"].get("disk_percent", 100) < 95
                and sql_success_rate >= 90  # SQL operations must be at least 90% successful
            )
            ...
            if overall_healthy:
                ...
                self.logger.warning(
                    "Health check detected issues - see health_status.jsonl for details"
                )
            ...
            health_results.update({"health_check_failed": True, "error_message": str(e)})
            ...
            try:
                if self.is_wake_hours:
                    ...
                    if self.error_counts["working_memory"] > 5:
                        self.logger.warning(
                            "High working memory error count, increasing sleep interval"
                        )
                        time.sleep(15)
                    else:
                        time.sleep(5)
    ...
    def stm_processing(self):
        ...
        success = self.run_dbt_command("dbt run --select short_term_memory --quiet", "stm.log")
    ...
    def start_working_memory_thread(self):
        ...
        if self.working_memory_thread is None or not self.working_memory_thread.is_alive():
            ...
            self.working_memory_thread = threading.Thread(
                target=self.working_memory_continuous, daemon=True
            )
