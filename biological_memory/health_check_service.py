...
class PostgreSQLCircuitBreaker:
        ...
        with self._lock:
            if self.state == "OPEN":
                if self._should_attempt_reset():
                    ...
                    return (
                        False,
                        f"Circuit breaker is OPEN. Service unavailable until {self.last_failure_time + timedelta(seconds=self.timeout_seconds)}",
                    )
    ...
    def _on_failure(self):
        ...
        if self.failure_count >= self.failure_threshold and self.state != "OPEN":
            self.state = "OPEN"
            self.logger.error(
                f"PostgreSQL circuit breaker opened after {self.failure_count} failures"
            )
...
class ComprehensiveHealthMonitor:
        ...
        if BIOLOGICAL_MONITORING_AVAILABLE:
            try:
                self.biological_monitor = get_biological_parameter_monitor(
                    base_path=str(self.base_path), dbt_project_dir=str(self.base_path)
                )
    ...
    def _setup_circuit_breakers(self):
        ...
        self.circuit_breakers["postgresql"] = PostgreSQLCircuitBreaker(
            failure_threshold=5, timeout_seconds=60, connection_params=postgres_params
        )

        # Ollama circuit breaker (enhanced)
        self.circuit_breakers["ollama"] = CircuitBreaker(
            failure_threshold=3, timeout_seconds=30, expected_exception=Exception
        )

        # DuckDB circuit breaker (enhanced)
        self.circuit_breakers["duckdb"] = CircuitBreaker(
            failure_threshold=3, timeout_seconds=20, expected_exception=Exception
        )
    ...
    def check_system_resources(self) -> HealthCheckResult:
        ...
        try:
            ...
            details = {
                ...
                "cpu": {"usage_percent": cpu_percent, "load_average": load_avg},
                "warnings": warnings,
            }
...
class HealthHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_detailed_health(self):
        ...
        health_results = {
            name: asdict(result) for name, result in self.health_monitor.health_results.items()
        }
    ...
    def _handle_alerts(self):
        ...
        alerts_data = {
            ...
            "recent_alerts": [asdict(alert) for alert in self.health_monitor.alert_history[-10:]],
        }
