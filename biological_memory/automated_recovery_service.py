...
from health_check_service import ServiceStatus, get_health_monitor
...
class AutomatedRecoveryService:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Automated recovery service initialized (enabled: {recovery_enabled}, dry_run: {dry_run})"
        )
    ...
    def _setup_recovery_rules(self) -> Dict[str, RecoveryRule]:
        ...
        return {
            ...
            "duckdb": RecoveryRule(
                ...
                failure_threshold=2,
                recovery_actions=[RecoveryAction.RESET_CONNECTIONS, RecoveryAction.CLEAR_CACHE],
                cooldown_seconds=60,
                max_attempts=5,
            ...
            "ollama": RecoveryRule(
                ...
                failure_threshold=2,
                recovery_actions=[RecoveryAction.RESTART_SERVICE, RecoveryAction.CLEAR_CACHE],
                cooldown_seconds=90,
                max_attempts=3,
                escalation_actions=[RecoveryAction.ESCALATE_ALERT],
            ),
    ...
    def start_automated_recovery(self):
        ...
        self.logger.info(
            f"Automated recovery monitoring started (interval: {self.check_interval}s)"
        )
    ...
    def _handle_service_failure(self, service_name: str, health_result: Any, rule: RecoveryRule):
        ...
        self.recovery_attempts[service_name] = [
            attempt for attempt in self.recovery_attempts[service_name] if attempt > cutoff_time
        ]
        ...
        self.logger.info(
            f"Triggering recovery for {service_name} (failure count: {self.service_failures[service_name]})"
        )
    ...
    def _execute_recovery_actions(self, service_name: str, actions: List[RecoveryAction]):
        ...
        for action in actions:
            try:
                ...
                if success:
                    self.logger.info(
                        f"Recovery action successful: {action.value} for {service_name}"
                    )
                else:
    ...
    def _restart_process(self, service_name: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"Process restart requested for {service_name} - manual intervention may be needed"
            )
            return True
    ...
    def _clear_cache(self, service_name: str) -> bool:
        ...
        try:
                ...
                if llm_cache_path.exists():
                    ...
                    try:
                        with duckdb.connect(str(llm_cache_path)) as conn:
                            conn.execute(
                                "DELETE FROM llm_cache WHERE accessed_at < CURRENT_TIMESTAMP - INTERVAL '1 HOUR'"
                            )
                        cache_cleared = True
            ...
            temp_dirs = [self.base_path / "target", self.base_path / "logs" / "dbt.log"]
                ...
                try:
                    ...
                    if cpu_percent > 90 or memory_percent > 50:
                        ...
                        if running_time > 300:  # 5 minutes
                            ...
                            killed_count += 1
                            self.logger.info(
                                f"Killed stuck process: {proc.info['name']} (PID: {proc.info['pid']})"
                            )
    ...
    def get_recovery_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recovery_attempts_last_hour": {
                service: len(attempts) for service, attempts in self.recovery_attempts.items()
            },
            "last_recovery_times": {
                service: last_time.isoformat() for service, last_time in self.last_recovery.items()
            },
