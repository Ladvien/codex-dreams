...
class TestRetryLogic:
    ...
    def test_exponential_backoff_with_jitter(self):
        ...
        for i in range(3):
            ...
            with pytest.raises(Exception):
                self.error_handler.exponential_backoff_retry(
                    failing_operation, max_retries=2, base_delay=0.1, jitter=True
                )
...
class TestDeadLetterQueueRecovery:
    ...
    def test_dead_letter_queue_batch_processing(self):
        ...
        successful_messages = [
            c for c in remaining_candidates if int(c["message_id"].split("_")[-1]) % 2 == 0
        ]
...
class TestRecoveryStrategies:
    ...
    def test_error_correlation_and_pattern_detection(self):
        ...
        connection_errors = [
            e for e in recent_errors if e.error_type == ErrorType.CONNECTION_FAILURE
        ]
