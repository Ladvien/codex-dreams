...
class PackageManagementTest:
    ...
    def test_packages_yml_exists_and_valid(self) -> bool:
        ...
        try:
            ...
            self.log_test("packages_yml_exists_and_valid", True, 
                         f"packages.yml is valid with {packages_count} packages", 
                         {"packages": packages_config['packages']})
            return True
            
        except Exception as e:
            self.log_test("packages_yml_exists_and_valid", False, f"Error reading packages.yml: {str(e)}")
            return False
    ...
    def test_package_lock_consistency(self) -> bool:
        ...
        try:
            ...
            if len(packages_list) != len(lock_packages_list):
                self.log_test("package_lock_consistency", False, 
            ...
            for pkg in packages_list:
                ...
                lock_pkg = next((p for p in lock_packages_list if p.get('package') == pkg_name), None)
                if not lock_pkg:
                    self.log_test("package_lock_consistency", False, 
            ...
            self.log_test("package_lock_consistency", True, 
                         "packages.yml and package-lock.yml are consistent", 
                         {"packages_match": True, "lock_has_security_hash": lock_has_hash})
            return True
            
        except Exception as e:
            self.log_test("package_lock_consistency", False, f"Error comparing package files: {str(e)}")
            return False
    ...
    def test_dbt_utils_version_compatibility(self) -> bool:
        ...
        try:
            ...
            if not dbt_version:
                self.log_test("dbt_utils_version_compatibility", False, "Could not parse dbt version")
                return False
            ...
            dbt_utils_pkg = next((pkg for pkg in packages_config['packages'] if 'dbt_utils' in pkg.get('package', '')), None)
            if not dbt_utils_pkg:
                self.log_test("dbt_utils_version_compatibility", False, "dbt_utils package not found in packages.yml")
                return False
            ...
            self.log_test("dbt_utils_version_compatibility", compatible,
                         f"dbt {dbt_version} compatibility with dbt_utils {dbt_utils_version}",
                         {"dbt_version": dbt_version, "dbt_utils_version": dbt_utils_version, "compatible": compatible})
            return compatible
            
        except Exception as e:
            self.log_test("dbt_utils_version_compatibility", False, f"Error checking version compatibility: {str(e)}")
            return False
    ...
    def test_duckdb_adapter_compatibility(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if not deps_success:
                    self.log_test("duckdb_adapter_compatibility", False, 
                ...
                self.log_test("duckdb_adapter_compatibility", deps_success and duckdb_mentioned,
                             f"DuckDB adapter compatibility verified",
                             {"deps_success": deps_success, "debug_success": debug_success, "duckdb_adapter_found": duckdb_mentioned})
            ...
            self.log_test("duckdb_adapter_compatibility", False, f"Error testing DuckDB compatibility: {str(e)}")
            return False
    ...
    def test_macro_compilation(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(['dbt', 'parse'], capture_output=True, text=True, timeout=60, env=test_env)
                ...
                self.log_test("macro_compilation", parse_success, message,
    ...
    def test_biological_specific_package_needs(self) -> bool:
        ...
        try:
            ...
            self.log_test("biological_specific_package_needs", True,
            ...
            self.log_test("biological_specific_package_needs", False, f"Error analyzing biological package needs: {str(e)}")
            return False
    ...
    def test_package_security_and_provenance(self) -> bool:
        ...
        try:
            if not self.package_lock_yml.exists():
                self.log_test("package_security_and_provenance", False, "package-lock.yml not found")
                return False
            ...
            for pkg in lock_config.get('packages', []):
                if 'sha1_hash' in pkg:
                    ...
                    if len(hash_val) != 40 or not all(c in '0123456789abcdef' for c in hash_val.lower()):
                        self.log_test("package_security_and_provenance", False, 
            ...
            self.log_test("package_security_and_provenance", security_acceptable,
            ...
            self.log_test("package_security_and_provenance", False, f"Error checking package security: {str(e)}")
            return False
