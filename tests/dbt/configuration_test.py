...
class TestDBTConfiguration:
    ...
    def test_biological_orchestration_tags(self):
        ...
        required_tags = ['continuous', 'short_term', 'consolidation', 'long_term']
    ...
    def test_ollama_variables(self):
        ...
        assert '192.168.1.110:11434' in ollama_url, "ollama_url should have correct default"
        
        # Test ollama_model has correct default
        ollama_model = vars_config.get('ollama_model', '')
        assert 'OLLAMA_MODEL' in ollama_model or ollama_model == 'llama2', "ollama_model should be 'llama2' or use env var"
    ...
    def test_biological_timing_parameters(self):
        ...
        assert vars_config.get('working_memory_duration') == 300, "working_memory_duration should be 300 seconds (5 minutes)"
        
        # Test working memory capacity parameters
        assert vars_config.get('working_memory_capacity_base') == 7, "working_memory_capacity_base should be 7"
        assert vars_config.get('working_memory_capacity_variance') == 2, "working_memory_capacity_variance should be 2"
        
        # Test short-term memory duration is 30 minutes (1800 seconds)
        assert vars_config.get('short_term_memory_duration') == 1800, "short_term_memory_duration should be 1800 seconds (30 minutes)"
    ...
    def test_incremental_strategy_configuration(self):
        ...
        assert stm_config.get('+unique_key') == 'memory_id', "STM should have memory_id as unique_key"
        ...
        assert cons_config.get('+unique_key') == 'memory_id', "Consolidation should have memory_id as unique_key"
        assert cons_config.get('+incremental_strategy') == 'merge', "Consolidation should use merge strategy"
    ...
    def test_biological_parameter_ranges(self):
        ...
        assert 0.01 <= hebbian_rate <= 0.5, f"Hebbian learning rate {hebbian_rate} outside biological range"
        
        # Consolidation threshold should be reasonable (0.3-0.8)
        cons_threshold = vars_config.get('consolidation_threshold', 0)
        assert 0.3 <= cons_threshold <= 0.8, f"Consolidation threshold {cons_threshold} outside biological range"
    ...
    def test_performance_hooks_configuration(self):
        ...
        assert any('threads' in hook for hook in pre_hooks), "Working memory should have thread optimization"
        
        # Check consolidation has memory optimization
        cons_config = models.get('consolidation', {})
        cons_pre_hooks = cons_config.get('+pre-hook', [])
        assert any('memory_limit' in hook for hook in cons_pre_hooks), "Consolidation should have memory limit"
    ...
    def test_on_run_hooks(self):
        ...
        assert 'biological memory' in start_hooks_str.lower(), "Start hooks should mention biological memory"
...
class TestBiologicalParameters:
    ...
    def test_hebbian_learning_parameters(self):
        ...
        assert 0.05 <= learning_rate <= 0.2, f"Learning rate {learning_rate} outside biological range"
