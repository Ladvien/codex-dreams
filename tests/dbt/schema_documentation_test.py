...
class TestSchemaDocumentation:
                ...
                if 'models' in schema_content and isinstance(schema_content['models'], list):
                    for i, model in enumerate(schema_content['models']):
                        if not isinstance(model, dict):
                            model_errors.append(f"{directory}/schema.yml: Model {i} must be a dictionary")
                            continue
                        
                        if 'name' not in model:
                            model_errors.append(f"{directory}/schema.yml: Model {i} missing 'name' field")
                        
                        if 'description' not in model:
                            model_errors.append(f"{directory}/schema.yml: Model {model.get('name', i)} missing 'description' field")
                        ...
                        if 'columns' in model and isinstance(model['columns'], list):
                            for i, column in enumerate(model['columns']):
                                if not isinstance(column, dict):
                                    column_errors.append(f"{directory}/{model_name}: Column {i} must be a dictionary")
                                    continue
                                
                                if 'name' not in column:
                                    column_errors.append(f"{directory}/{model_name}: Column {i} missing 'name' field")
                                
                                if 'description' not in column:
                                    column_errors.append(f"{directory}/{model_name}: Column {column.get('name', i)} missing 'description' field")
                        ...
                        if 'columns' in model:
                            ...
                            for column in model['columns']:
                                ...
                                if column_name in ['id', 'memory_id']:
                                    ...
                                    if not has_not_null:
                                        test_coverage_issues.append(f"{directory}/{model_name}: Column {column_name} missing not_null test")
                                    
                                    if not has_unique:
                                        test_coverage_issues.append(f"{directory}/{model_name}: Column {column_name} missing unique test")
    ...
    def test_strength_columns_have_range_tests(self):
        ...
        range_test_issues = []
        strength_columns = [
            'activation_strength', 'stm_strength', 'consolidated_strength',
            'hebbian_strength', 'recency_score', 'frequency_score',
            'emotional_salience', 'consolidation_priority', 'stability_score',
            'importance_score', 'decay_resistance'
        ]
                        ...
                        if 'columns' in model:
                            for column in model['columns']:
                                ...
                                if column_name in strength_columns:
                                    has_range_test = any(
                                        'accepted_range' in str(test) or 'dbt_utils.accepted_range' in str(test)
                                        for test in tests
                                    )
                                    
                                    if not has_range_test:
                                        range_test_issues.append(f"{directory}/{model_name}: Column {column_name} missing range test")
                    ...
                    if 'models' in schema_content:
                        ...
                        for model_name in model_names:
                            if model_name not in documented_models:
                                coverage_issues.append(f"{directory}: Model {model_name}.sql not documented in schema.yml")
