...
class DbtConfigurationTest(unittest.TestCase):
    ...
    def test_ollama_configuration_variables(self):
        ...
        for var_name, expected_default in required_ollama_vars.items():
            self.assertIn(var_name, self.vars_config, 
    ...
    def test_biological_timing_parameters(self):
        ...
        for param, expected_value in biological_params.items():
            self.assertIn(param, self.vars_config, 
                         f"Missing biological parameter: {param}")
            self.assertEqual(self.vars_config[param], expected_value,
    ...
    def test_biological_orchestration_tags(self):
        ...
        for model_group, required_tags in expected_tags.items():
            self.assertIn(model_group, self.models_config,
                         f"Missing model group: {model_group}")
            
            model_config = self.models_config[model_group]
            self.assertIn('+tags', model_config,
                         f"Missing +tags in {model_group} configuration")
            
            actual_tags = model_config['+tags']
            self.assertIsInstance(actual_tags, list,
                                f"{model_group} tags should be a list")
            
            for tag in required_tags:
                self.assertIn(tag, actual_tags,
                             f"Missing required tag '{tag}' in {model_group}")
    ...
    def test_model_materialization_strategies(self):
        ...
        for model_group, expected_mat in expected_materializations.items():
            if model_group in self.models_config:
                model_config = self.models_config[model_group]
                self.assertIn('+materialized', model_config,
                             f"Missing +materialized in {model_group}")
                actual_mat = model_config['+materialized']
                self.assertEqual(actual_mat, expected_mat,
    ...
    def test_incremental_configuration(self):
        ...
        for model in incremental_models:
            if model in self.models_config:
                ...
                self.assertEqual(config.get('+materialized'), 'incremental',
                               f"{model} should be incremental")
                
                self.assertIn('+unique_key', config,
                             f"{model} missing unique key")
                
                if '+incremental_strategy' in config:
                    self.assertEqual(config['+incremental_strategy'], 'merge',
    ...
    def test_memory_thresholds_configuration(self):
        ...
        for threshold in required_thresholds:
            self.assertIn(threshold, self.vars_config,
                         f"Missing threshold: {threshold}")
            value = self.vars_config[threshold]
            self.assertIsInstance(value, (int, float),
                                f"{threshold} should be numeric")
            self.assertGreaterEqual(value, 0,
                                  f"{threshold} should be non-negative")
            self.assertLessEqual(value, 1,
                               f"{threshold} should be <= 1")
    ...
    def test_hebbian_learning_parameters(self):
        ...
        for param, expected in hebbian_params.items():
            self.assertIn(param, self.vars_config,
                         f"Missing Hebbian parameter: {param}")
            self.assertEqual(self.vars_config[param], expected,
                           f"{param} should be {expected}")
    
    def test_configuration_completeness(self):
        """Test that configuration includes all required sections."""
        required_sections = [
            'name', 'version', 'config-version', 'profile',
            'model-paths', 'vars', 'models'
        ]
    ...
    def test_environment_variable_security(self):
        ...
        for var in sensitive_vars:
            if var in self.vars_config:
                ...
                self.assertIn('env_var(', var_value,
                             f"{var} should use env_var() for security")
                # Should not contain hardcoded values
                self.assertNotIn('localhost', var_value,
