...
class TestBiologicalTiming:
    ...
    def test_biological_constraint_consistency(self):
        ...
        assert (
            stm_duration > wm_duration
        ), f"STM duration ({stm_duration}s) should be longer than WM duration ({wm_duration}s)"

        # Consolidation interval should be reasonable (multiple STM cycles acceptable)
        # Allow up to 4 STM cycles for consolidation (following sleep-wake research)
        max_consolidation_interval = stm_duration * 4  # 2 hours max
        assert consolidation_interval <= max_consolidation_interval, (
            f"Consolidation interval ({consolidation_interval}s) should be <= {max_consolidation_interval}s "
            f"(4x STM duration). Research basis: consolidation can span multiple STM cycles"
        )
...
class TestMemoryStageTransitionTiming:
    ...
    def test_short_term_to_long_term_transition(self):
        ...
        stm_duration = self.vars.get("short_term_memory_duration", 0)
        self.vars.get("consolidation_threshold", 0)
        consolidation_interval = self.vars.get("consolidation_interval_hours", 0) * 3600

        # Multiple STM cycles feed into each consolidation cycle (biologically accurate)
        stm_cycles_per_consolidation = consolidation_interval / stm_duration
        assert stm_cycles_per_consolidation >= 1, (
            f"Consolidation should span multiple STM cycles. "
            f"STM duration: {stm_duration}s, consolidation interval: {consolidation_interval}s, "
            f"ratio: {stm_cycles_per_consolidation:.1f} STM cycles per consolidation"
        )
