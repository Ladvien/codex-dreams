...
class TestOrganization:
    ...
    def setup_class(cls):
        ...
        cls.dbt_utils_tests = cls.project_root / "biological_memory" / "dbt_packages" / "dbt_utils" / "integration_tests" / "tests"
    ...
    def test_minimal_file_duplication(self):
        ...
        assert duplicates == expected_duplicates, f"Unexpected duplicates found: {duplicates - expected_duplicates}"
    ...
    def test_directory_purpose_separation(self):
        ...
        expected_main = {"analytics", "database", "integration", "orchestration", "infrastructure", "memory", "performance", "reliability"}
        
        infrastructure_dirs = main_subdirs.intersection(expected_main)
        assert len(infrastructure_dirs) >= 5, f"Main tests should have infrastructure focus, found: {main_subdirs}"
        
        # Biological tests should focus on dbt/model validation
        bio_subdirs = {d.name for d in self.bio_tests.iterdir() if d.is_dir()}
        expected_bio = {"dbt", "parameter_validation", "performance", "long_term", "short_term_memory", "extension"}
        
        bio_focus_dirs = bio_subdirs.intersection(expected_bio)
        assert len(bio_focus_dirs) >= 3, f"Bio tests should have dbt/model focus, found: {bio_subdirs}"
    ...
    def test_conftest_files_serve_different_purposes(self):
        ...
        assert any(keyword in main_content.lower() for keyword in ["database connections", "ollama", "test data fixtures"]), \
            "Main conftest should focus on infrastructure fixtures"
            
        # Bio conftest should focus on biological/model testing
        assert any(keyword in bio_content.lower() for keyword in ["biological", "model", "duckdb test database"]), \
    ...
    def test_reasonable_file_counts(self):
        ...
        assert 20 <= len(main_py_files) <= 60, f"Main tests count seems unreasonable: {len(main_py_files)}"
        assert 20 <= len(bio_py_files) <= 60, f"Bio tests count seems unreasonable: {len(bio_py_files)}"
        ...
        assert total_files < 115, f"Total test files ({total_files}) contradicts '115+ duplicates' claim"
    ...
    def test_no_consolidation_needed(self):
        ...
        infrastructure_keywords = ["integration", "orchestration", "database", "connection", "environment"]
        main_infra_count = sum(1 for name in main_names 
                              for keyword in infrastructure_keywords 
                              if keyword in name.lower())
        
        # Bio tests should have model/biological-focused names
        bio_keywords = ["biological", "dbt", "schema", "validation", "parameter"]
        bio_model_count = sum(1 for name in bio_names 
                             for keyword in bio_keywords 
                             if keyword in name.lower())
        
        assert main_infra_count >= 3, f"Main tests should have infrastructure focus: {main_infra_count}"
