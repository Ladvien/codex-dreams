...
class TestEnvironmentConfiguration:
    ...
    def test_specific_env_values(self, test_env_vars):
        ...
        assert os.getenv('OLLAMA_MODEL') == 'gpt-oss:20b', \
            "OLLAMA_MODEL should be gpt-oss:20b"
        assert os.getenv('EMBEDDING_MODEL') == 'nomic-embed-text', \
            "EMBEDDING_MODEL should be nomic-embed-text"
        assert os.getenv('MAX_DB_CONNECTIONS') == '160', \
            "MAX_DB_CONNECTIONS should be 160"
    ...
    def test_database_url_format(self):
        ...
        for url in [test_db_url, postgres_db_url]:
            if url:
                assert url.startswith('postgresql://'), f"URL should use postgresql:// scheme: {url}"
...
class TestEnvironmentValidation:
    ...
    def test_numeric_env_var_validation(self):
        ...
        numeric_vars = [
            ('MAX_DB_CONNECTIONS', 1, 1000),
            ('OLLAMA_TIMEOUT', 60, 600)
        ]
