...
from services.incremental_processor import IncrementalProcessor, ProcessingState, IncrementalBatch
...
class TestWritebackInfrastructure:
    ...
    def test_schema_creation(self, test_db_connection):
        ...
        with test_db_connection.cursor() as cursor:
            ...
            schema_file = Path(__file__).parent.parent.parent / 'sql' / 'create_codex_processed_schema.sql'
            ...
            for table in required_tables:
                ...
                assert cursor.fetchone() is not None, f"Table {table} should exist in codex_processed schema"
    ...
    def test_table_constraints(self, test_db_connection):
        ...
        with test_db_connection.cursor() as cursor:
            ...
            with pytest.raises(psycopg2.IntegrityError):
                ...
                """, (str(uuid.uuid4()), 1.5))  # Invalid strength > 1.0
...
class TestMemoryWritebackService:
    ...
    def test_extract_processed_memories(self, mock_execute, mock_writeback_service):
        ...
        mock_result.fetchall.return_value = [
            (str(uuid.uuid4()), 'Test Goal', ['task1'], ['action1'], {}, 0.8, 'cortical_transfer', 0.7, 'semantic test', 'strategic', 'prefrontal_cortex', 0.9, 0.6, 0.5, ['concept1'], datetime.now(timezone.utc), '1.0')
        ]
        
        # Mock connection description
        service.duckdb_conn.description = [
            ('source_memory_id',), ('level_0_goal',), ('level_1_tasks',), ('level_2_actions',), 
            ('phantom_objects',), ('consolidated_strength',), ('consolidation_fate',), 
            ('hebbian_strength',), ('semantic_gist',), ('semantic_category',), 
            ('cortical_region',), ('retrieval_accessibility',), ('stm_strength',), 
            ('emotional_salience',), ('concepts',), ('processed_at',), ('processing_version',)
        ]
...
class TestDBTIntegration:
    ...
    def test_dbt_results_validation(self):
        ...
        failed_results = {
            'success': False,
            'results': [
                {'unique_id': 'model.test.memory_replay', 'status': 'error', 'message': 'SQL compilation error'}
            ]
        }
...
class TestPerformanceAndScalability:
    ...
    def test_batch_processing_performance(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert processing_time < 1.0, f"Batch size {batch_size} took too long: {processing_time}s"
    ...
    def test_concurrent_processing_safety(self):
        ...
        for i in range(5):
            worker = threading.Thread(
                target=mock_processing_worker,
                args=(i, records_per_worker)
            )
