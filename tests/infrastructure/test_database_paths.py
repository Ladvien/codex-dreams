...
class DatabasePathValidator:
    ...
    def validate_environment_variable_usage(self) -> List[str]:
        ...
        orchestrator_file = (
            self.project_root / "biological_memory" / "orchestrate_biological_memory.py"
        )
        if orchestrator_file.exists():
            ...
            if "os.getenv('DUCKDB_PATH'" not in content:
                issues.append(
                    "orchestrate_biological_memory.py should use DUCKDB_PATH environment variable"
                )

            # Should use OLLAMA_URL environment variable
            if "os.getenv('OLLAMA_URL'" not in content:
                issues.append(
                    "orchestrate_biological_memory.py should use OLLAMA_URL environment variable"
                )
...
def test_required_environment_variables_documented():
    ...
    required_vars = ["DUCKDB_PATH", "OLLAMA_URL", "POSTGRES_DB_URL", "LLM_CACHE_PATH"]
    ...
    if missing_vars:
        ...
        pytest.fail(
            f"Required environment variables not documented in .env.example:\n{missing_list}"
        )
...
def test_orchestrator_constructor_uses_env_vars():
    ...
    assert (
        "_validate_environment_variables" in content
    ), "Orchestrator should validate environment variables"

    # Should use DBT_PROJECT_DIR for base path
    assert (
        "os.getenv('DBT_PROJECT_DIR'" in content
    ), "Orchestrator should use DBT_PROJECT_DIR environment variable"

    # Should use DUCKDB_PATH
    assert (
        "os.getenv('DUCKDB_PATH'" in content
    ), "Orchestrator should use DUCKDB_PATH environment variable"
