...
class ConfigurationConsistencyTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.required_vars = {
            ...
            "EMBEDDING_MODEL", "DUCKDB_PATH", "DBT_PROFILES_DIR", 
            "DBT_PROJECT_DIR", "MAX_DB_CONNECTIONS"
        }
    ...
    def test_model_name_consistency(self):
        ...
        self.assertIsNotNone(production_model_match, 
                           "OLLAMA_MODEL not found in .env.example")
        self.assertEqual(production_model_match.group(1), self.expected_production_model,
        ...
        if self.env_testing_path.exists():
            ...
            self.assertIsNotNone(test_model_match, 
                               "OLLAMA_MODEL not found in .env.testing")
            self.assertEqual(test_model_match.group(1), self.expected_test_model,
    ...
    def test_embedding_model_consistency(self):
        ...
        for file_path in files_to_check:
            ...
            self.assertIsNotNone(embedding_match, 
                               f"EMBEDDING_MODEL not found in {file_path.name}")
            self.assertEqual(embedding_match.group(1), self.expected_embedding_model,
        ...
        for file_path in files_to_check:
            ...
            for timeout_var in self.standardized_timeout_vars:
                if timeout_var in content:
                    ...
                    self.assertIsNotNone(var_match, 
    ...
    def test_no_hardcoded_ip_addresses(self):
        ...
        for file_path in files_to_check:
            ...
            self.assertEqual([], hardcoded_ips, 
    ...
    def test_documentation_consistency(self):
        ...
        if self.readme_path.exists():
            ...
            self.assertIn(self.expected_production_model, readme_content,
            ...
            config_section_match = re.search(r'```.*?POSTGRES_DB_URL.*?```', readme_content, re.DOTALL)
    ...
    def test_environment_variable_documentation(self):
        ...
        self.assertEqual([], poorly_documented_vars,
    ...
    def test_production_vs_test_environment_separation(self):
        ...
        if self.env_testing_path.exists():
            ...
            test_timeout_match = re.search(r'OLLAMA_GENERATION_TIMEOUT_SECONDS=([0-9]+)', test_content)
            if test_timeout_match:
                test_timeout = int(test_timeout_match.group(1))
                
                prod_timeout_match = re.search(r'OLLAMA_GENERATION_TIMEOUT_SECONDS=([0-9]+)', prod_content)
                if prod_timeout_match:
                    prod_timeout = int(prod_timeout_match.group(1))
                    self.assertLessEqual(test_timeout, prod_timeout,
