...
class TestParallelCapabilities:
    ...
    def test_concurrent_database_connections(self):
        ...
        def create_isolated_connection(worker_id):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=f"_worker_{worker_id}.duckdb") as f:
                db_path = f.name
...
class TestPerformanceOptimizations:
    ...
    def test_fixture_setup_time(self, performance_benchmark):
        ...
        assert avg_setup < 10, f"Average fixture setup time ({avg_setup:.2f}ms) should be under 10ms"
    ...
    def test_parallel_speedup_measurement(self):
        ...
        assert speedup_ratio > 1.5, f"Parallel execution should show speedup (ratio: {speedup_ratio:.2f})"
    ...
    def test_resource_cleanup_efficiency(self):
        ...
        def create_and_cleanup_resources(worker_id):
            ...
            for i in range(5):
                with tempfile.NamedTemporaryFile(delete=False, suffix=f"_w{worker_id}_f{i}.tmp") as f:
    ...
    def test_large_scale_parallel_execution(self):
        ...
        assert avg_teardown < 0.005, f"Average teardown time ({avg_teardown:.4f}s) should be under 5ms"
        ...
        assert speedup > 2.0, f"Large scale parallel execution should show 2x+ speedup (actual: {speedup:.2f}x)"