...
class TestHebbianLearningBiologicalAccuracy:
    ...
    def test_hebbian_formula_mathematical_accuracy(self):
        ...
        expected_weight = initial_weight * (
            1.0 + learning_rate * (pre_synaptic_activity * normalized_post_activity)
        )

        # Validate calculation
        calculated_weight = initial_weight * (
            1.0 + learning_rate * (pre_synaptic_activity * normalized_post_activity)
        )

        assert (
            abs(calculated_weight - expected_weight) < 1e-6
    ...
    def test_learning_rate_biological_bounds(self):
        ...
        """
        assert (
            self.min_learning_rate <= self.biological_learning_rate <= self.max_learning_rate
    ...
    def test_spike_timing_dependent_plasticity_simulation(self):
        ...
        strengthened_weight = initial_weight * (
            1.0 + learning_rate * (pre_activity_strong * normalized_post)
        )
        ...
        weakly_strengthened = initial_weight * (
            1.0 + learning_rate * (pre_activity_weak * normalized_post_strong)
        )

        # Should strengthen less than positive STDP case
        assert (
            weakly_strengthened < strengthened_weight
    ...
    def test_synaptic_weight_normalization(self):
        ...
        for cycle in range(10):  # Multiple learning cycles
            ...
            current_weight = current_weight * (
                1.0 + learning_rate * (pre_activity * normalized_post)
            )
    ...
    def test_co_activation_strengthening_correlation(self):
        ...
        high_strengthened = initial_weight * (
            1.0 + learning_rate * (stm_strength * normalized_high)
        )

        # Higher co-activation should result in stronger synapses
        assert (
            high_strengthened > low_strengthened
        ...
        assert (
            low_strengthened > initial_weight and high_strengthened > initial_weight
    ...
    def test_null_safety_biological_defaults(self):
        ...
        assert 0.0 < result < 1.0, f"NULL defaults should produce valid biological result: {result}"

        # Should be slight strengthening from baseline
        assert result > null_hebbian, "Even minimal activity should produce slight strengthening"
...
def test_hebbian_implementation_matches_research_formula(hebbian_test_data):
    ...
    for synapse in hebbian_test_data["test_synapses"]:
        ...
        assert (
            result >= initial * hebbian_test_data["expected_ranges"]["min_strengthening"]
        ), f"Synapse {synapse} strengthening too weak: {result}"

        assert (
            result <= initial * hebbian_test_data["expected_ranges"]["max_strengthening"]
...
if __name__ == "__main__":
    ...
    pytest.main([__file__, "-v", "--tb=short", "-k", "test_hebbian", "--capture=no"])
