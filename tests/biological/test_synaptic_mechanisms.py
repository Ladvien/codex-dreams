...
from tests.fixtures.database import duckdb_connection, test_database_setup
from tests.fixtures.mocking import mock_ollama_service
...
class TestAdvancedSynapticMechanisms:
    ...
    def test_stdp_temporal_windows(self, duckdb_connection):
        ...
        test_cases = [
            {'delay_ms': 15, 'expected_type': 'stdp_potentiation', 'expected_strength': 1.0},  # Optimal
            {'delay_ms': 35, 'expected_type': 'stdp_potentiation', 'expected_strength': 0.7},  # Strong LTP
            {'delay_ms': -55, 'expected_type': 'stdp_depression', 'expected_strength': -0.5}, # LTD
            {'delay_ms': 120, 'expected_type': 'no_stdp_effect', 'expected_strength': 0.0},   # No effect
        ]
        ...
        if len(depression_rows) > 0:
            assert all(depression_rows['stdp_strength_factor'] < 0), "LTD should have negative strength"
    ...
    def test_ltp_ltd_differential_strengthening(self, duckdb_connection):
        ...
        coactivation_data = [
            ...
            {'pre_id': 'strong_memory', 'post_id': 'medium_memory', 'coactivation_count': 5, 
             'stdp_window_type': 'stdp_potentiation', 'stdp_strength_factor': 1.0},
            # LTD scenario - weak connection
            {'pre_id': 'weak_memory', 'post_id': 'medium_memory', 'coactivation_count': 2,
             'stdp_window_type': 'stdp_depression', 'stdp_strength_factor': -0.5},
            # Standard Hebbian - no STDP effect
            {'pre_id': 'medium_memory', 'post_id': 'strong_memory', 'coactivation_count': 3,
             'stdp_window_type': 'no_stdp_effect', 'stdp_strength_factor': 0.0},
        ]
        ...
        duckdb_connection.execute("CREATE TABLE spike_timing_analysis AS SELECT * FROM coactivation_df")
        ...
        if len(strong_memory_effect) > 0:
            assert strong_memory_effect.iloc[0]['ltp_ltd_delta'] > 0, "Strong memories should potentiate"
    ...
    def test_bcm_metaplasticity_factors(self, duckdb_connection):
        ...
        assert high_activity['metaplasticity_threshold'] > 0.8, "High activity should raise threshold"
        ...
        assert normal_activity['metaplasticity_threshold'] == 0.5, "Normal activity should keep threshold"
    ...
    def test_synaptic_tagging_and_capture(self, duckdb_connection):
        ...
        for _, tagged in tagged_memories.iterrows():
            assert tagged['coactivation_count'] >= 3, "Tagged memories need sufficient co-activation"
        ...
        for _, untagged in untagged_memories.iterrows():
            assert (untagged['coactivation_count'] < 3 or 
                   untagged['stdp_strength_factor'] <= 0.5), "Untagged memories fail criteria"
    ...
    def test_homeostatic_synaptic_scaling(self, duckdb_connection):
        ...
        for _, row in result.iterrows():
            assert abs(row['scaled_synaptic_change']) <= max_allowed_change * 1.1, \
    ...
    def test_winner_take_all_competition(self, duckdb_connection):
        ...
        assert all(moderates['competition_factor'] == 0.5), "Moderates should get partial strengthening"
        ...
        for i in range(len(sorted_result) - 1):
            ...
            assert current_factor >= next_factor, "Competition factors should decrease with activity"
    ...
    def test_advanced_synaptic_integration(self, duckdb_connection, mock_ollama_service):
        ...
        duckdb_connection.execute("CREATE TABLE stm_hierarchical_episodes AS SELECT * FROM memory_df")
        duckdb_connection.execute("""
        ...
        assert 'metaplasticity_threshold' in integration_result.columns, "Should calculate metaplasticity"
        ...
        assert all(integration_result['metaplasticity_threshold'] >= 0.4), "Metaplasticity thresholds in range"
        assert all(integration_result['metaplasticity_threshold'] <= 1.0), "Metaplasticity thresholds in range"
        assert all(integration_result['competition_factor'].isin([0.2, 0.5, 1.0])), "Competition factors valid"
    ...
    def test_synaptic_mechanism_performance(self, duckdb_connection):
        ...
        duckdb_connection.execute("CREATE TABLE performance_test_memories AS SELECT * FROM memory_df")
...
class TestSynapticParameterValidation:
    ...
    def test_research_citation_compliance(self):
        ...
        for mechanism, details in research_compliance.items():
            assert details['verified'], f"{mechanism} implementation should comply with {details['citation']}"
