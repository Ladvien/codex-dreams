...
class TestREMCreativeProcessing:
    ...
    def test_rem_simulation_targets_creative_models(self, mock_getcwd, mock_chdir, mock_run):
        ...
        assert (
            "concept_associations" in model_str
        ), "Should target concept associations for creativity"
    ...
    def test_rem_simulation_failure_handling(self, mock_run):
        ...
        mock_run.side_effect = subprocess.CalledProcessError(
            1, "dbt", stderr="Creative processing failed"
        )
    ...
    def test_wagner_insight_generation_timing(self):
        ...
        for hour in creative_hours:
            with patch("src.orchestration.biological_rhythm_scheduler.datetime") as mock_dt:
                ...
                assert (
                    phase == CircadianPhase.REM_DOMINANT
...
class TestREMSleepFailureRecovery:
    ...
    def test_rem_simulation_exception_recovery(self):
        ...
        self.scheduler.processor.rem_sleep_simulation.side_effect = Exception(
            "Creative processing error"
        )
    ...
    def test_rem_blocked_during_wake_hours(self):
        ...
        for hour, phase in wake_times:
            with patch("src.orchestration.biological_rhythm_scheduler.datetime") as mock_dt:
                ...
                assert (
                    should_run is False
                ), f"REM should be blocked at {hour}:00 during {phase.value}"
    ...
    def test_rem_blocked_during_non_rem_sleep(self):
        ...
        for hour, phase in non_rem_sleep_times:
            with patch("src.orchestration.biological_rhythm_scheduler.datetime") as mock_dt:
                ...
                assert (
                    should_run is False
                ), f"REM should be blocked during {phase.value} at {hour}:00"
