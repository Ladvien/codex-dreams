...
class TestBiologicalTimingConstraints:
    ...
    def test_continuous_timing_miller_law(self):
        ...
        for test_time, should_run in test_cases:
            with patch('orchestration.biological_rhythm_scheduler.datetime') as mock_dt:
                ...
                # Simulate wake hours
                self.scheduler._get_current_circadian_phase = Mock(return_value=CircadianPhase.WAKE_ACTIVE)
                
                result = self.scheduler._should_run_continuous()
                assert result == should_run, f"At {test_time}, should_run_continuous should be {should_run}"
    ...
    def test_continuous_blocked_during_sleep(self):
        ...
        for phase in sleep_phases:
            with patch('orchestration.biological_rhythm_scheduler.datetime') as mock_dt:
                ...
                result = self.scheduler._should_run_continuous()
                assert result is False, f"Continuous processing should be blocked during {phase.value}"
    ...
    def test_short_term_20_minute_cycles(self):
        ...
        for test_time, should_run in test_cases:
            with patch('orchestration.biological_rhythm_scheduler.datetime') as mock_dt:
                ...
                assert result == should_run, f"At {test_time}, should_run_short_term should be {should_run}"
    ...
    def test_long_term_90_minute_ultradian_cycles(self):
        ...
        for test_time, should_run in test_cases:
            with patch('orchestration.biological_rhythm_scheduler.datetime') as mock_dt:
                ...
                assert result == should_run, f"At {test_time}, should_run_long_term should be {should_run}"
    ...
    def test_deep_sleep_consolidation_timing(self):
        ...
        with patch('orchestration.biological_rhythm_scheduler.datetime') as mock_dt:
            ...
            self.scheduler._get_current_circadian_phase = Mock(return_value=CircadianPhase.DEEP_SLEEP)
            # Simulate next day
