                ...
                with open(py_file, 'r', encoding='utf-8') as f:
                    for line_no, line in enumerate(f, 1):
                        if len(line.rstrip()) > max_length:
                            violations.append(f"{py_file}:{line_no} - Line too long ({len(line.rstrip())} > {max_length})")
            except Exception:
        ...
        for py_file in formatter.project_root.rglob("*.py"):
            ...
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    for line_no, line in enumerate(f, 1):
                        if line.startswith('\t'):
                            violations.append(f"{py_file}:{line_no} - Tab character used instead of spaces")
                        elif line.startswith(' '):
                            leading_spaces = len(line) - len(line.lstrip(' '))
                            if leading_spaces % 4 != 0:
                                violations.append(f"{py_file}:{line_no} - Inconsistent indentation ({leading_spaces} spaces)")
            except Exception:
                continue
