...
class TypeHintChecker:
    ...
    def _check_function_annotations(self, node: ast.FunctionDef, file_path: Path) -> List[str]:
        ...
        if (
            ...
            or node.name in ["main", "setUp", "tearDown"]
        ):
            return issues
        ...
        if node.returns is None and node.name not in ["__init__"]:
            issues.append(
                f"{file_path}:{node.lineno} - Function '{node.name}' missing return type annotation"
            )
        ...
        for arg in node.args.args:
            if arg.arg not in ["self", "cls"] and arg.annotation is None:
                issues.append(
                    f"{file_path}:{node.lineno} - Parameter '{arg.arg}' in '{node.name}' missing type annotation"
                )
...
class TestTypeHints:
    ...
    def test_src_directory_type_hints(self, type_checker):
        ...
        src_files = [f for f in type_checker.get_python_files() if "src/" in str(f)]
    ...
    def test_biological_memory_type_hints(self, type_checker):
        ...
        bio_files = [
            f
            for f in type_checker.get_python_files()
    ...
    def test_complex_types_properly_imported(self, type_checker):
        ...
        for file_path in all_files:
            try:
                ...
                if (
                    has_list_type or has_dict_type or has_optional_type or has_union_type
                ) and not has_typing_import:
                    import_issues.append(
                        f"{file_path} uses complex types but missing typing import"
                    )
