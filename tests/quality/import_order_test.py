...
class ImportOrderChecker:
    ...
    def _classify_import(self, import_info: Dict) -> str:
        ...
        stdlib_modules = {
            ...
            're', 'hashlib', 'uuid', 'pickle', 'subprocess', 'threading',
            'asyncio', 'concurrent', 'enum', 'abc'
        }
        
        # Local modules (project-specific)
        local_patterns = ['src', 'biological_memory', 'macros', 'dreams_writeback_service']
...
class TestImportOrder:
    ...
    def test_src_directory_import_order(self, import_checker):
        ...
        src_files = [
            f for f in import_checker.get_python_files()
            if 'src/' in str(f)
        ]
            ...
            try:
                ...
                if 'import *' in content:
                    for line_no, line in enumerate(content.split('\n'), 1):
                        if 'import *' in line:
                            wildcard_violations.append(f"{file_path}:{line_no} - Wildcard import: {line.strip()}")
                ...
                for line_no, import_line in import_lines:
                    ...
                    if 'import ' in import_line:
                        ...
                        if imported_name and imported_name not in content.replace(import_line, ''):
                            potential_unused.append(f"{file_path}:{line_no} - Potentially unused: {imported_name}")
