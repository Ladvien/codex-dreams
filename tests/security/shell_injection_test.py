...
class TestShellInjectionPrevention:
        ...
        for dangerous_cmd in dangerous_commands:
            ...
            for char in orchestrator.dangerous_chars:
                if char in dangerous_cmd:
                    assert '[FILTERED]' in sanitized, f"Dangerous character '{char}' should be filtered"
                    assert char not in sanitized, f"Original dangerous character '{char}' should be removed"
    ...
    def test_subprocess_execution_security(self, orchestrator):
        ...
        with patch('subprocess.run') as mock_subprocess:
            ...
            with patch.object(orchestrator, 'error_handler') as mock_error_handler:
                ...
                assert call_args[1]['shell'] is False, "shell=False must be used to prevent injection"
    ...
    def test_comprehensive_security_regression(self, orchestrator):
        ...
        assert blocked_count == len(attack_vectors), f"All {len(attack_vectors)} attack vectors should be blocked"
        
        # Verify security logging occurred
        assert orchestrator.logger.error.call_count >= len(attack_vectors), "Security events should be logged"
    ...
    def test_timeout_security(self, mock_subprocess, orchestrator):
        ...
        with patch.object(orchestrator, 'error_handler') as mock_error_handler:
            mock_error_handler.exponential_backoff_retry.side_effect = subprocess.TimeoutExpired("cmd", 5)
