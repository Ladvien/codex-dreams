...
class TestMemoryPerformance:
    ...
    def test_working_memory_refresh_performance(self, performance_benchmark, working_memory_fixture):
    ...
    @pytest.mark.performance
    def test_short_term_memory_processing_performance(self, performance_benchmark, short_term_memory_fixture):
    ...
    def test_memory_search_performance(self, performance_benchmark, working_memory_fixture):
        ...
        for i in range(20):
            ...
            """, (i+100, f'Search test memory {i}', datetime.now(timezone.utc), '{"source": "search_test"}'))
    ...
    @pytest.mark.performance
    def test_consolidation_candidate_selection_performance(self, performance_benchmark, short_term_memory_fixture):
        ...
        # Performance requirement: <100ms
        assert timer.elapsed < 0.1, f"Consolidation selection took {timer.elapsed:.3f}s, should be <0.1s"
    ...
    def test_hebbian_strength_calculation_performance(self, performance_benchmark, test_duckdb):
        ...
        # Performance requirement: <50ms
        assert timer.elapsed < 0.05, f"Hebbian calculation took {timer.elapsed:.3f}s, should be <0.05s"
    ...
    def test_full_pipeline_integration_performance(self, performance_benchmark, test_duckdb):
        ...
        for i in range(10):
            conn.execute("""
                INSERT INTO raw_memories VALUES (?, ?, ?, ?)
            """, (i, f'Pipeline test memory {i}', datetime.now(timezone.utc), '{"source": "pipeline"}'))
        ...
        with performance_benchmark() as timer:
            ...
            for memory in raw_data:
                ...
                """, (memory[0], memory[1], processed_goal, processed_tasks, processed_actions, strength))
...
class TestMemoryCapacityLimits:
    ...
    def test_capacity_overflow_handling_performance(self, performance_benchmark, test_duckdb):
        ...
        assert timer.elapsed < 0.05, f"Overflow handling took {timer.elapsed:.3f}s, should be <0.05s"  
