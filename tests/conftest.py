    ...
    for i, memory in enumerate(sample_memory_data):
        ...
        """,
            (
                ...
                f"Goal {i+1}",
                f"Tasks for goal {i+1}",
                f"Actions for goal {i+1}",
                "objects",
                "spatial_info",
                0.5 + i * 0.1,
                i + 1,
...
def isolated_test_db(test_duckdb, test_postgres_connection):
    ...
    return {"duckdb": test_duckdb, "postgres": test_postgres_connection}
    ...
    for i in range(1000):
        ...
        """,
            (
                i + 1,
                content,
                json.dumps({"batch": i // 100, "importance": random.uniform(0.3, 0.9)}),
            ),
        )
