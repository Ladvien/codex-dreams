...
# Test configuration
DUCKDB_PATH = os.getenv('DUCKDB_PATH', '/Users/ladvien/codex-dreams/biological_memory/dbs/memory.duckdb')
POSTGRES_URL = os.getenv('POSTGRES_DB_URL', 'postgresql://codex_user:M|h!y,3:tL^-MJSRswpH09N_JJnNkj?Q@192.168.1.104:5432/codex_db')
...
class TestBiologicalMemorySchema:
    ...
    def test_episodic_buffer_table_structure(self, duckdb_connection):
        ...
        required_columns = [
            'id', 'original_content', 'level_0_goal', 'level_1_tasks', 
            'atomic_actions', 'spatial_context', 'temporal_context',
            'social_context', 'phantom_objects', 'temporal_marker',
            'causal_links', 'stm_strength', 'hebbian_potential',
            'consolidation_priority', 'ready_for_consolidation',
            'entered_wm_at', 'entered_stm_at', 'wm_duration_seconds'
        ]
    ...
    def test_consolidation_buffer_table_structure(self, duckdb_connection):
        ...
        required_columns = [
            'id', 'original_content', 'level_0_goal', 'level_1_tasks',
            'discovered_patterns', 'synthesized_insights', 'predictive_patterns',
            'abstract_principles', 'contradictions', 'associated_memory_ids',
            'association_strengths', 'total_association_strength',
            'semantic_gist', 'semantic_category', 'cortical_region',
            'knowledge_type', 'abstraction_level', 'pre_consolidation_strength',
            'consolidated_strength', 'entered_stm_at', 'consolidated_at',
    ...
    def test_semantic_memory_table_structure(self, postgres_connection):
        ...
        required_columns = [
            'id', 'source_memory_id', 'semantic_gist', 'knowledge_type',
            'abstraction_level', 'confidence_score', 'semantic_category',
            'cortical_region', 'patterns', 'insights', 'predictions',
            'principles', 'contradictions', 'related_concepts',
            'integration_points', 'retrieval_strength', 'access_count',
            'last_accessed_at', 'consolidation_strength', 'hebbian_weight',
            'synaptic_stability', 'consolidated_during_sleep',
            'rem_sleep_associations', 'consolidation_cycle_type',
            'processing_version', 'created_at', 'search_vector'
        ]
    ...
    def test_biological_constraints_validation(self, duckdb_connection):
        ...
        with pytest.raises(Exception):
            cursor.execute("""
                ...
                VALUES (?, 'test content', ?)
            """, [str(uuid.uuid4()), 1.5])  # Should fail - exceeds 1.0
    ...
    def test_consolidation_abstraction_level_constraint(self, duckdb_connection):
        ...
        with pytest.raises(Exception):
            cursor.execute("""
                ...
                VALUES (?, ?)
            """, [str(uuid.uuid4()), 6])  # Should fail - exceeds 5
    ...
    def test_required_indexes_exist(self, duckdb_connection):
        ...
        for index in expected_episodic_indexes:
            assert any(index in idx_name for idx_name in episodic_indexes), \
        ...
        for index in expected_consolidation_indexes:
            assert any(index in idx_name for idx_name in consolidation_indexes), \
    ...
    def test_millers_law_capacity_logic(self, duckdb_connection):
        ...
        cursor.execute("""
            ...
            WHERE id IN ({})
        """.format(','.join(['?' for _ in test_episodes])), test_episodes)
        
        count = cursor.fetchone()[0]
        assert count == 12, "Table should support more than Miller's limit for processing flexibility"
        ...
        for episode_id in test_episodes:
            cursor.execute("DELETE FROM biological_memory.episodic_buffer WHERE id = ?", [episode_id])
    ...
    def test_temporal_constraints_and_indexing(self, duckdb_connection):
        ...
        cursor.execute("""
            ...
            WHERE id IN ({}) 
            AND entered_stm_at >= ?
        """.format(','.join(['?' for _ in test_episodes])), test_episodes + [five_min_ago])
        
        recent_count = cursor.fetchone()[0]
        assert recent_count == 3, f"Should find 3 episodes within 5-minute window, found {recent_count}"
    ...
    def test_consolidation_priority_calculation(self, duckdb_connection):
        ...
        """.format(','.join(['?' for id_, _, _ in test_ids])), [id_ for id_, _, _ in test_ids])
        ...
        assert highest_priority_id == test_ids[0][0], "High strength episode should have highest priority"
...
class TestSchemaIntegration:
    ...
    def test_schema_file_syntax(self):
        ...
        assert 'biological_memory.consolidation_buffer' in content, "Should define consolidation_buffer"
        assert 'codex_processed.semantic_memory' in content, "Should define semantic_memory"
        
        # Check for biological constraints
        assert 'stm_strength >= 0.0 AND stm_strength <= 1.0' in content, "Should have STM strength constraints"
        assert 'abstraction_level >= 1 AND abstraction_level <= 5' in content, "Should have abstraction level constraints"
    ...
    def test_architecture_compliance(self):
        ...
        for requirement in architecture_requirements:
            assert requirement in content, f"Architecture requirement {requirement} should be implemented"
