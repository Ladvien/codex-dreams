...
class ConnectionSecurityTests(unittest.TestCase):
    ...
    def test_environment_variable_usage(self):
        ...
        self.assertGreater(
            len(files_with_env_usage), 0,
            "No files found using proper environment variable patterns for database configuration"
        )
        
        print(f"✅ Found {len(files_with_env_usage)} files using proper environment variable patterns")
    ...
    def test_secure_connection_templates(self):
        ...
        self.assertGreater(
            secure_usage_count, insecure_usage_count,
            f"Found more insecure patterns ({insecure_usage_count}) than secure ones ({secure_usage_count})"
        )
        
        print(f"✅ Security pattern analysis: {secure_usage_count} secure, {insecure_usage_count} insecure")
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if '=' in line:
                        ...
                        if key.upper() in ['POSTGRES_PASSWORD', 'DATABASE_PASSWORD']:
                            self.assertIn(
                                value.lower(), ['your_password_here', 'change_me', 'defaultpassword', 'password'],
