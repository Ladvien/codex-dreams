...
class BiologicalMemoryPipelineTester:
    ...
    def call_ollama_with_fallback(self, prompt: str, processing_type: str = "extraction") -> Dict[str, Any]:
        ...
        try:
            ...
            model_to_use = None
            for preferred_model in [self.config.ollama_model, "qwen2.5:0.5b", available_models[0] if available_models else None]:
    ...
    def setup_test_data_in_postgres(self, num_memories: int = 10) -> List[Tuple[int, str, datetime, str]]:
            ...
            with conn.cursor() as cursor:
                ...
                for i, (content, importance) in enumerate(test_memories[:num_memories]):
                    ...
                    metadata = {
                        ...
                        "source": "e2e_test",
                        "category": ["planning", "development", "review", "client", "operations"][i % 5],
                    ...
                    inserted_memories.append((memory_id, content, test_timestamp, json.dumps(metadata)))
    ...
    def test_complete_memory_lifecycle_with_live_services(self):
        ...
        with self.pipeline:
            ...
            with self.pipeline.get_integrated_duckdb() as duckdb_conn:
                ...
                """, [self.pipeline.config.working_memory_capacity]).fetchall()
                
                wm_processing_time = time.perf_counter() - start_time
                
                # Biological constraints validation
                assert wm_processing_time < 0.1, f"Working memory processing took {wm_processing_time:.3f}s, should be <0.1s"
                assert len(working_memory_result) <= self.pipeline.config.working_memory_capacity, "Should respect Miller's 7±2 capacity"
                assert len(working_memory_result) >= 5, "Should select high-importance memories"
                
                logger.info(f"Working Memory: {len(working_memory_result)} items selected in {wm_processing_time:.3f}s")
...
Client Relations and Service, Operations and System Maintenance"""
                    ...
                    llm_result = self.pipeline.call_ollama_with_fallback(hierarchy_prompt, "hierarchy_analysis")
                    ...
                    ready_for_consolidation = stm_strength >= self.pipeline.config.consolidation_threshold
                    ...
                    duckdb_conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, (memory_id, content, goal, tasks, actions, stm_strength, hebbian_potential, ready_for_consolidation))
                ...
                for candidate in consolidation_candidates:
                    ...
                    for concept_a, concept_b, assoc_strength in associations:
                        if concept_a and concept_b and len(concept_a.strip()) > 0 and len(concept_b.strip()) > 0:
                            duckdb_conn.execute("""
                ...
                logger.info(f"Long-Term Memory: {ltm_associations_created} semantic associations created")
                ...
                assert pipeline_stats["working_memory"] <= self.pipeline.config.working_memory_capacity
                assert pipeline_stats["stm_episodes"] <= pipeline_stats["working_memory"]
                assert pipeline_stats["ltm_associations"] >= pipeline_stats["consolidation_candidates"]
    ...
    def test_biological_timing_constraints_end_to_end(self):
        ...
        with self.pipeline:
            ...
            with self.pipeline.get_integrated_duckdb() as duckdb_conn:
                ...
                assert wm_time < 0.1, f"Working memory processing took {wm_time:.3f}s, should be <0.1s"
                assert stm_time < 0.5, f"STM processing took {stm_time:.3f}s, should be <0.5s"
                assert consolidation_time < 2.0, f"Consolidation took {consolidation_time:.3f}s, should be <2.0s"
                assert total_pipeline_time < 5.0, f"Total pipeline took {total_pipeline_time:.3f}s, should be <5.0s"
    ...
    def test_hebbian_learning_with_live_services(self):
        ...
        with self.pipeline:
            ...
            with self.pipeline.get_integrated_duckdb() as duckdb_conn:
                ...
                working_memories = duckdb_conn.execute(f"""
                ...
                for memory_id, content in working_memories:
                    ...
                    entity_result = self.pipeline.call_ollama_with_fallback(entity_prompt, "entity_extraction")
                ...
                assert max_strength > 0.6, f"Max association strength {max_strength} should show Hebbian strengthening"
                
                logger.info(f"Hebbian learning: {len(strengthened_associations)} associations strengthened")
                for assoc in strengthened_associations[:3]:
                    logger.info(f"  {assoc[0]} <-> {assoc[1]}: strength={assoc[2]:.3f}, retrievals={assoc[3]}")
    ...
    def test_error_recovery_in_complete_pipeline(self):
        ...
        with self.pipeline:
            ...
            with self.pipeline.get_integrated_duckdb() as duckdb_conn:
                ...
                assert successful_stm_processing >= 2, "Should process most memories despite LLM failures"
                
                # Test consolidation continues
                stm_episodes = duckdb_conn.execute("SELECT COUNT(*) FROM stm_episodes").fetchall()[0][0]
                ...
                ltm_associations = duckdb_conn.execute("SELECT COUNT(*) FROM ltm_semantic_network").fetchall()[0][0]
                assert ltm_associations >= 1, "Should create LTM associations despite service issues"
                
                logger.info(f"Error recovery: {successful_stm_processing} STM episodes, {ltm_associations} LTM associations")
