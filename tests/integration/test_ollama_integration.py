...
class OllamaIntegrationTester:
    ...
    def call_ollama_generate(self, prompt: str, model: Optional[str] = None, timeout: Optional[int] = None) -> Dict[str, Any]:
...
class TestOllamaConnectivity:
    ...
    def test_ollama_required_models_available(self):
        ...
        primary_available = any(self.tester.config.primary_model in model for model in available_models)
        embedding_available = any(self.tester.config.embedding_model in model for model in available_models)
...
Format: {{"goal": "category", "confidence": 0.0-1.0}}"""
            ...
            if "error" not in result:
                ...
                try:
                    ...
                    assert response_time < 10.0, f"Goal extraction took {response_time:.3f}s, should be <10.0s"
                    
                except json.JSONDecodeError:
                    # In test environment, model might not return perfect JSON
                    logger.warning(f"Non-JSON response (acceptable in test): {response_text[:50]}...")
    ...
    def test_ollama_embedding_generation_for_semantic_network(self):
        ...
        for concept in test_concepts:
            ...
            if "error" not in result and "embedding" in result:
                ...
                assert response_time < 15.0, f"Embedding took {response_time:.3f}s, should be <15.0s"
                
                logger.info(f"Generated embedding for '{concept}': {len(embedding)} dimensions in {response_time:.3f}s")
                
            else:
                logger.warning(f"Embedding failed for {concept}: {result.get('error', 'Unknown error')}")
...
class TestOllamaCachingAndPerformance:
    ...
    def test_ollama_caching_system_performance(self):
        ...
        with self.tester.get_test_cache_db() as cache_db:
            ...
            for prompt in test_prompts:
                ...
                """, (prompt_hash, test_model)).fetchall()
                
                if cached_result:
                    # Cache hit
                    cache_hits += 1
                    response_time = time.perf_counter() - start_time
                    logger.info(f"Cache hit for prompt: {prompt[:30]}... ({response_time*1000:.1f}ms)")
                    ...
                    if "error" not in ollama_result:
                        ...
                        logger.info(f"Cache miss for prompt: {prompt[:30]}... ({response_time:.3f}s)")
                    else:
            ...
            if cache_hits + cache_misses > 0:
                ...
                if cache_hits > 0:
                    ...
                    assert fastest_response < 0.05, f"Fastest response was {fastest_response:.3f}s, cache should be <0.05s"
    ...
    def test_ollama_concurrent_request_handling(self):
        ...
        successful_results = [r for r in results if "error" not in r and "error" not in r.get("result", {})]
        
        if successful_results:
            logger.info(f"Concurrent requests: {len(successful_results)}/{len(results)} successful in {total_time:.3f}s")
            
            # All successful requests should complete in reasonable time
            for result in successful_results:
                assert result["time"] < 15.0, f"Request {result['id']} took {result['time']:.3f}s, should be <15.0s"
        else:
            logger.warning("No successful concurrent requests - this may be normal for test environment")
    ...
    def test_ollama_comprehensive_health_check(self):
        ...
        try:
            ...
            health_status['models'] = {
                ...
                'primary_model_available': any(self.tester.config.primary_model in model['name'] for model in models),
                'embedding_model_available': any(self.tester.config.embedding_model in model['name'] for model in models),
            ...
            if health_status['models']['has_any_models']:
                ...
                health_status['generation'] = {
                    ...
                    'response_preview': gen_result.get('response', '')[:50] if 'response' in gen_result else 'N/A'
                }
            else:
                health_status['generation'] = {
                    'successful': False,
            ...
            assert health_status['response_time'] < 2.0, f"Health check too slow: {health_status['response_time']:.3f}s"
            assert health_status['models']['has_any_models'], "No models available"
            
            if health_status['generation']['successful']:
                assert health_status['generation']['response_time'] < 10.0, "Generation test too slow"
            ...
            logger.info(f"Generation test: {'✅' if health_status['generation']['successful'] else '❌'}")
