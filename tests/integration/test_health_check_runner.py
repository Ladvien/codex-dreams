...
class IntegrationHealthChecker:
    ...
    def check_postgresql_health(self) -> ServiceHealth:
        ...
        logger.info(
            f"Checking PostgreSQL health at {self.config.postgres_host}:{self.config.postgres_port}"
        )
    ...
    def check_ollama_health(self) -> ServiceHealth:
        ...
        try:
            ...
            response = requests.get(
                f"{self.config.ollama_url}/api/tags", timeout=self.config.ollama_timeout
            )
            ...
            primary_model_available = any(
                self.config.ollama_model in model for model in available_models
            )
            embedding_model_available = any(
                self.config.ollama_embedding_model in model for model in available_models
            )
            ...
            # Warning, not error, as this is optional
