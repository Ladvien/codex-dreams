...
class BiologicalMemoryPerformanceTester:
    ...
    def measure_operation_performance(self, operation_name: str, operation_func, num_iterations: int = 10, 
                                    biological_constraint_ms: float = 50.0) -> PerformanceMetrics:
    ...
    def setup_performance_test_data(self, num_records: int = 1000):
        ...
        with self.get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(f"CREATE INDEX idx_perf_importance ON {perf_table} (importance_score DESC)")
                cursor.execute(f"CREATE INDEX idx_perf_category ON {perf_table} (category)")
                cursor.execute(f"CREATE INDEX idx_perf_metadata ON {perf_table} USING GIN (metadata)")
                
                # Insert test data in batches for better performance
                batch_size = 100
                categories = ["planning", "development", "review", "client", "operations", "research"]
                ...
                for batch_start in range(0, num_records, batch_size):
                    ...
                    for i in range(batch_start, min(batch_start + batch_size, num_records)):
                        ...
                        metadata = json.dumps({
                            ...
                            "priority": "high" if importance > 0.8 else "medium" if importance > 0.5 else "low"
                        })
                        
                        batch_values.append((content, test_timestamp, importance, category, metadata))
    ...
    def test_working_memory_query_performance(self):
        ...
        with self.tester:
            ...
            with self.tester.get_performance_duckdb() as conn:
                ...
                assert metrics.biological_constraint_met, f"Working memory query took {metrics.avg_time_ms:.1f}ms, should be <{metrics.constraint_ms}ms"
                assert metrics.success_rate >= 0.95, f"Success rate {metrics.success_rate:.2f} too low"
                assert metrics.throughput_ops_per_sec >= 20, f"Throughput {metrics.throughput_ops_per_sec:.1f} ops/sec too low"
                
                logger.info(f"Working Memory Performance: {metrics.avg_time_ms:.1f}ms avg, {metrics.p95_time_ms:.1f}ms p95")
    ...
    def test_short_term_memory_processing_performance(self):
        ...
        with self.tester:
            ...
            with self.tester.get_performance_duckdb() as conn:
                ...
                assert metrics.biological_constraint_met, f"STM processing took {metrics.avg_time_ms:.1f}ms, should be <{metrics.constraint_ms}ms"
                assert metrics.success_rate >= 0.9, f"STM success rate {metrics.success_rate:.2f} too low"
                
                logger.info(f"STM Processing Performance: {metrics.avg_time_ms:.1f}ms avg, {metrics.p95_time_ms:.1f}ms p95")
    ...
    def test_consolidation_query_performance(self):
        ...
        with self.tester:
            ...
            with self.tester.get_performance_duckdb() as conn:
                ...
                assert metrics.biological_constraint_met, f"Consolidation took {metrics.avg_time_ms:.1f}ms, should be <{metrics.constraint_ms}ms"
                assert metrics.success_rate >= 0.85, f"Consolidation success rate {metrics.success_rate:.2f} too low"
                
                logger.info(f"Consolidation Performance: {metrics.avg_time_ms:.1f}ms avg, {metrics.p95_time_ms:.1f}ms p95")
...
class TestLLMServicePerformance:
    ...
    def test_ollama_response_time_performance(self):
        ...
        if metrics.success_rate > 0:
            ...
            assert metrics.biological_constraint_met, f"LLM generation took {metrics.avg_time_ms:.1f}ms, should be <5000ms"
            assert metrics.success_rate >= 0.8, f"LLM success rate {metrics.success_rate:.2f} too low"
            
            logger.info(f"LLM Performance: {metrics.avg_time_ms:.1f}ms avg, {metrics.p95_time_ms:.1f}ms p95")
        else:
    ...
    def test_embedding_generation_performance(self):
        ...
        try:
            ...
            embedding_models = [model['name'] for model in available_models if 'embed' in model['name'].lower()]
        ...
        if metrics.success_rate > 0:
            assert metrics.biological_constraint_met, f"Embedding took {metrics.avg_time_ms:.1f}ms, should be <10000ms"
...
class TestConcurrentPerformance:
    ...
    def test_concurrent_database_connections(self):
        ...
        with self.tester:
            ...
            assert len(successful_results) >= num_threads * 0.8, f"Only {len(successful_results)}/{num_threads} concurrent queries succeeded"
            ...
            if successful_results:
                ...
                assert avg_query_time < self.tester.config.database_query_limit_ms * 2, f"Concurrent avg time {avg_query_time:.1f}ms too slow"
                assert max_query_time < self.tester.config.database_query_limit_ms * 3, f"Concurrent max time {max_query_time:.1f}ms too slow"
                ...
                logger.info(f"Average query time: {avg_query_time:.1f}ms, Max: {max_query_time:.1f}ms")
    ...
    def test_memory_capacity_under_load(self):
        ...
        with self.tester:
            ...
            with self.tester.get_performance_duckdb() as conn:
                ...
                assert avg_time < self.tester.config.database_query_limit_ms * 1.5, f"Capacity queries too slow: {avg_time:.1f}ms"
                
                logger.info(f"Capacity under load: consistent {max(capacities)} ≤ 7 items, {avg_time:.1f}ms avg")
    ...
    def test_attention_window_processing_performance(self):
        ...
        with self.tester:
            ...
            with self.tester.get_performance_duckdb() as conn:
                ...
                assert metrics.biological_constraint_met, f"Attention window processing took {metrics.avg_time_ms:.1f}ms, should be <{metrics.constraint_ms}ms"
                assert metrics.success_rate >= 0.95, "Attention window processing should be highly reliable"
    ...
    def test_forgetting_curve_application_performance(self):
        ...
        with self.tester:
            ...
            with self.tester.get_performance_duckdb() as conn:
                ...
                assert metrics.biological_constraint_met, f"Forgetting curve took {metrics.avg_time_ms:.1f}ms, should be <200ms"
                ...
                if len(strength_data) >= 2:
                    assert strength_data[0][1] > strength_data[-1][1], "Forgetting curve should weaken older memories"
...
def generate_performance_report(test_results: List[PerformanceMetrics]) -> str:
    ...
    def add_operation_section(operations: List[PerformanceMetrics], title: str):
        ...
        for op in operations:
            ...
            report.append(f"    Performance: {op.avg_time_ms:.1f}ms avg, {op.p95_time_ms:.1f}ms p95")
