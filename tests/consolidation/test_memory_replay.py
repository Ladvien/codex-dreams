...
class TestMemoryReplayConsolidation:
    ...
    def test_basic_consolidation_pipeline(self, simple_duckdb):
        ...
        assert memory_2[4] == 'hippocampal_retention', "Low-strength memory should stay in hippocampus"
    ...
    def test_hebbian_strengthening_algorithm(self, simple_duckdb):
        ...
        """.format(','.join(f"({id_}, {pot}, {exp})" for id_, pot, exp in test_data)))
        ...
        for row in result:
            assert row[3], f"Hebbian calculation incorrect for ID {row[0]}: got {row[2]}, expected close to {float(row[1]) * 1.1}"
    ...
    def test_competitive_forgetting_mechanism(self, simple_duckdb):
        ...
        """.format(','.join(f"({id_}, {strength}, {exp})" for id_, strength, exp in test_cases)))
        ...
        for row in result:
            assert row[4], f"Competitive forgetting incorrect for ID {row[0]}: got {row[2]}, expected {row[3]}"
    ...
    def test_consolidation_performance_benchmark(self, simple_duckdb):
        ...
        assert processing_time < 1.0, f"Consolidation took {processing_time:.3f}s, should be <1s for {batch_size} memories"
    ...
    def test_biological_accuracy_validation(self, simple_duckdb):
        ...
        for query, expected, description in tests:
            ...
            assert abs(result_float - expected) < 0.001, f"{description}: got {result_float}, expected {expected}"
...
class TestConsolidationOptimization:
    ...
    def test_batch_processing_efficiency(self, simple_duckdb):
        ...
        for batch_size in batch_sizes:
            ...
            assert processing_time < (batch_size / 100.0), f"Batch {batch_size} too slow: {processing_time:.3f}s"
