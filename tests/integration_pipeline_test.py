...
class TestMemoryPipelineIntegration:
    ...
    def test_complete_memory_lifecycle(self, memory_lifecycle_data, mock_ollama):
        ...
        assert ltm_count[0][0] >= len(ready_for_consolidation), "Memories should be consolidated to LTM"
    ...
    def test_cross_component_data_consistency(self, memory_lifecycle_data, mock_ollama):
        ...
        assert len(wm_with_raw) == total_wm[0][0], \
        ...
        assert len(stm_with_source) == total_stm[0][0], \
        ...
        assert temporal_consistency[0][0] == total_stm[0][0], \
    ...
    def test_performance_under_load(self, performance_test_data, mock_ollama):
        ...
        assert wm_selection_time < 0.1, f"WM selection took {wm_selection_time:.3f}s, should be <0.1s"
    ...
    def test_biological_accuracy_validation(self, biological_memory_schema, mock_ollama):
        ...
        assert old_strength < recent_strength, "Older memories should be weaker due to forgetting curve"
