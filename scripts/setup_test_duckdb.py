...
def setup_test_duckdb():
    ...
    test_postgres_url = os.getenv('TEST_DATABASE_URL', 'postgresql://codex_user:test_password@localhost:5432/test_codex_db')
    ...
    try:
        ...
        if match:
            ...
            attach_string = f"host={host} port={port} dbname={database} user={user} password={password}"
        else:
