...
class DreamsWritebackService:
    ...
    def __init__(self):
        ...
        self.duckdb_path = os.getenv(
            "DUCKDB_PATH",
            "/Users/ladvien/biological_memory/dbs/memory.duckdb"
        )
    ...
    def write_working_memory(self):
        ...
        try:
            ...
            for idx, memory in enumerate(memories):
                memory_id, content, timestamp, metadata, activation, access_count, summary, tags = memory
            ...
            pg_conn.commit()
            logger.info(f"Successfully wrote {len(memories)} working memory records (snapshot: {snapshot_id})")
    ...
    def write_long_term_memories(self):
        ...
        try:
            ...
            # Record metrics
            self._record_metrics(pg_cursor, 'long_term_memories', 
                                len(consolidatable), len(consolidatable), 0)
            pg_conn.commit()
    ...
    def write_semantic_network(self):
        ...
        try:
            ...
            # Record metrics
            self._record_metrics(pg_cursor, 'semantic_network',
                                len(associations), len(associations), 0)
            pg_conn.commit()
    ...
    def cleanup_old_data(self, days_to_keep: int = 30):
        ...
        try:
            ...
            pg_conn.commit()
            logger.info(f"Cleaned up {deleted_wm} working memory records and {deleted_metrics} metrics")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "working":
            ...
            print("Usage: dreams_writeback_service.py [working|episodes|longterm|semantic|insights|cleanup|full]")
    else:
