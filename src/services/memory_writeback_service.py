...
class MemoryWritebackService:
    ...
    def __init__(self, 
        ...
        self.postgres_url = postgres_url or os.getenv('POSTGRES_DB_URL', 'postgresql://codex_user:password@localhost:5432/codex_db')
        ...
        self.logger.info(f"Memory write-back service initialized with session {self.current_session_id}")
    ...
    def write_processed_memories(self, batch_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Processed memories write-back completed: {metrics.successful_writes}/{metrics.memories_processed} successful")
    ...
    def _write_processed_memories_batch(self, memories: List[Dict[str, Any]], metrics: ProcessingMetrics):
        ...
        with self._get_pg_connection() as pg_conn:
            with pg_conn.cursor() as cursor:
                try:
                    ...
                    for i in range(0, len(memories), self.batch_size):
                        ...
                        for memory in batch:
                            ...
                            data = {
                                ...
                                'retrieval_accessibility': memory.get('retrieval_accessibility', 0.0),
                                'memory_status': 'processed',
                                'processing_stage': 'complete',
                                'processed_at': memory.get('processed_at', datetime.now(timezone.utc)),
                    ...
                    pg_conn.commit()
                    self.logger.info(f"Successfully wrote {metrics.successful_writes} processed memories to PostgreSQL")
        ...
        try:
            ...
            for row in result:
                ...
                insights.append({
                    ...
                    'insight_confidence': min(0.8, insight_dict.get('connection_count', 0) / 10.0),
                    'novelty_score': 0.5,
                    'relevance_score': 0.7,
    ...
    def _write_insights_batch(self, insights: List[Dict[str, Any]], batch_id: str):
        ...
        with self._get_pg_connection() as pg_conn:
            with pg_conn.cursor() as cursor:
                try:
                    psycopg2.extras.execute_batch(
                        cursor, insert_query, insights, page_size=100
                    )
                    pg_conn.commit()
        ...
        try:
            ...
            for row in result:
                ...
                associations.append({
                    ...
                    'shared_concepts': [assoc_dict.get('source_concept'), assoc_dict.get('target_concept')],
                    ...
                    'discovered_at': assoc_dict.get('last_updated_at', datetime.now(timezone.utc))
                })
    ...
    def _write_associations_batch(self, associations: List[Dict[str, Any]], batch_id: str):
        ...
        with self._get_pg_connection() as pg_conn:
            with pg_conn.cursor() as cursor:
                try:
                    psycopg2.extras.execute_batch(
                        cursor, insert_query, associations, page_size=100
                    )
                    pg_conn.commit()
                    self.logger.info(f"Successfully wrote {len(associations)} associations to PostgreSQL")
    ...
    def write_processing_metadata(self, batch_id: str, additional_metadata: Dict[str, Any] = None):
        ...
        metadata = {
            ...
            'completion_percentage': (metrics.successful_writes / max(metrics.memories_processed, 1)) * 100
        }
    ...
    def run_full_writeback_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Full write-back cycle completed successfully in {results['total_duration']:.2f}s")
...
def main():
    ...
    parser.add_argument('--stage', choices=['memories', 'insights', 'associations', 'full'], 
                       default='full', help='Processing stage to run')
