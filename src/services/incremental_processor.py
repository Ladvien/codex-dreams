...
class IncrementalProcessor:
    ...
    def __init__(self, 
        ...
        self.pg_conn_params = self._parse_postgres_url(self.postgres_url) if self.postgres_url else None
    ...
    def _get_pg_connection(self):
        ...
        conn = None
        try:
            conn = psycopg2.connect(**self.pg_conn_params, cursor_factory=psycopg2.extras.DictCursor)
            yield conn
        finally:
            if conn:
                conn.close()
            ...
            with self._get_pg_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if row:
                        ...
                        self.logger.info(f"Loaded processing state for {stage_name}: last processed {state.last_processed_timestamp}")
                        return state
    ...
    def create_incremental_batch(self, 
                                ...
                                max_records: int = 10000) -> Optional[IncrementalBatch]:
        """
        ...
        if not since_timestamp:
            ...
            if state:
                ...
                since_timestamp = datetime.now(timezone.utc) - timedelta(hours=self.default_lookback_hours)
        ...
        self.logger.info(f"Created incremental batch {batch_id} with {len(data)} records for {stage}")
        return batch
    
    def _get_incremental_memories(self, since_timestamp: datetime, max_records: int) -> List[Dict[str, Any]]:
    ...
    def _get_incremental_insights(self, since_timestamp: datetime, max_records: int) -> List[Dict[str, Any]]:
    ...
    def _get_incremental_associations(self, since_timestamp: datetime, max_records: int) -> List[Dict[str, Any]]:
        ...
        try:
            with self._get_pg_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if result:
                        self.logger.info(f"Content hash {new_batch.batch_hash[:8]} already processed for {stage} at {result['processing_end_time']}")
                        return False
                    ...
                    cursor.execute(overlap_query, (stage, new_batch.watermark_start, new_batch.watermark_end))
                    overlap_count = cursor.fetchone()[0]
                    
                    if overlap_count > 0:
                        self.logger.warning(f"Found {overlap_count} overlapping processing windows for {stage}")
    ...
    def update_processing_state(self, 
        ...
        if success:
            state = ProcessingState(
                stage_name=stage,
                last_processed_timestamp=datetime.fromisoformat(batch.watermark_end.replace('Z', '+00:00')),
            ...
            self.logger.info(f"Updated processing state for {stage}: watermark={state.watermark_value}")
        else:
            self.logger.error(f"Processing failed for batch {batch.batch_id} in stage {stage}")
    
    def get_recovery_batches(self, 
                           stage: str, 
                           failed_timestamp: datetime,
                           max_recovery_hours: int = 72) -> List[IncrementalBatch]:
        """
        ...
        self.logger.info(f"Creating recovery batches for {stage} from {recovery_start} to {failed_timestamp}")
            ...
            with self._get_pg_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if result:
                        optimization = {
                            'stage': stage,
                            'current_avg_batch_size': float(result['avg_batch_size']) if result['avg_batch_size'] else 0,
                            'current_avg_duration': float(result['avg_duration']) if result['avg_duration'] else 0,
                            'failure_rate': float(result['failure_count']) / float(result['batch_count']) if result['batch_count'] > 0 else 0,
                        ...
                        if optimization['failure_rate'] > 0.1:  # More than 10% failure rate
                            optimization['recommendations'].append({
                                ...
                                'suggested_batch_size': max(500, int(optimization['current_avg_batch_size'] * 0.7))
                            })
                        ...
                        if optimization['current_avg_duration'] > 300:  # More than 5 minutes
                            optimization['recommendations'].append({
                                ...
                                'suggested_actions': ['increase_parallelism', 'optimize_queries', 'add_indexes']
                            })
                        ...
                        if optimization['current_avg_batch_size'] < 100:
                            optimization['recommendations'].append({
                                ...
                                'suggested_batch_size': min(2000, int(optimization['current_avg_batch_size'] * 2))
                            })
