...
def validate_dbt_success(dbt_result_path: str = None) -> bool:
    ...
    try:
        ...
        failed_nodes = [node for node in results.get('results', []) if node.get('status') != 'success']
...
def determine_processing_stages(dbt_models_executed: list = None) -> list:
    ...
    return list(executed_stages) if executed_stages else ['processed_memories', 'generated_insights', 'memory_associations']
    ...
    try:
        ...
        for stage in stages:
            ...
            try:
                ...
                results['total_records_processed'] += stage_result.get('memories_processed', 0) + \
                                                   stage_result.get('insights_generated', 0) + \
                                                   stage_result.get('associations_created', 0)
                ...
                if incremental and batch:
                    incremental_processor.update_processing_state(
                        ...
                        batch=batch,
                        success=True,
                        records_processed=stage_result.get('memories_processed', batch.record_count)
                    )
                ...
                writeback_service.write_processing_metadata(
                    ...
                    additional_metadata={
                        ...
                        'dbt_triggered': True,
                        'stage_duration_seconds': (datetime.now(timezone.utc) - stage_start).total_seconds()
                    }
                )
        ...
        results['total_duration_seconds'] = (results['integration_end'] - start_time).total_seconds()
        results['overall_status'] = 'completed' if results['total_errors'] == 0 else 'completed_with_errors'
        ...
        logger.info(f"Write-back integration completed: {results['total_records_processed']} records processed in {results['total_duration_seconds']:.2f}s")
...
def main():
    ...
    parser.add_argument('--dbt-results', 
                       help='Path to dbt run results JSON file')
    parser.add_argument('--force', action='store_true',
                       help='Force processing even if no new data')
    parser.add_argument('--log-level', default='INFO',
                       choices=['DEBUG', 'INFO', 'WARNING', 'ERROR'],
                       help='Logging level')
    parser.add_argument('--create-hook-script', 
