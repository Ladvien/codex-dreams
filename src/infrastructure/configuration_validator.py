...
class ConfigurationValidator:
    ...
    def __init__(self):
        ...
        self.optional_vars = {
            "DBT_PROFILES_DIR", "DBT_PROJECT_DIR", "MAX_DB_CONNECTIONS",
            "OLLAMA_GENERATION_TIMEOUT_SECONDS", "EMBEDDING_TIMEOUT_SECONDS",
            "LLM_CACHE_PATH", "TEST_DATABASE_URL"
        }
    ...
    def validate_environment_variables(self) -> bool:
        ...
        if missing_required:
            self.validation_errors.append(f"Missing required environment variables: {missing_required}")
        ...
        for var in db_vars:
            ...
            try:
                ...
                if not parsed.scheme:
                    ...
                    self.validation_errors.append(f"{var}: Expected postgresql:// scheme, got {parsed.scheme}://")
    ...
    def _validate_ollama_configuration(self):
        ...
        if ollama_model:
            ...
            if ollama_model not in [self.expected_models["production"], self.expected_models["testing"]]:
                ...
                if var.endswith("_DIR"):
                    ...
                    if path.exists() and not path.is_dir():
                        ...
                        if not path.parent.exists():
                            self.validation_warnings.append(f"{var}: Parent directory does not exist")
                else:
                    # File variables
        ...
        for var, default_value in self.timeout_defaults.items():
            ...
            if value_str:
                try:
                    value = int(value_str)
                    if value <= 0:
                        self.validation_errors.append(f"{var}: Timeout must be positive, got {value}")
                    elif value > 3600:  # 1 hour
                        self.validation_warnings.append(f"{var}: Very long timeout ({value}s), may cause delays")
                    elif value < 10:
                        self.validation_warnings.append(f"{var}: Very short timeout ({value}s), may cause failures")
    ...
    def validate_configuration_consistency(self) -> bool:
        ...
        if missing_in_example:
            self.validation_errors.append(f"Variables missing from .env.example: {missing_in_example}")
        
        # Check for hardcoded IP addresses
        hardcoded_ips = re.findall(r'192\.168\.1\.\d+', env_example_content)
        if hardcoded_ips:
            self.validation_errors.append(f"Hardcoded IP addresses in .env.example: {hardcoded_ips}")
        ...
        if env_testing_path.exists():
            ...
            if test_hardcoded_ips:
                self.validation_errors.append(f"Hardcoded IP addresses in .env.testing: {test_hardcoded_ips}")
    ...
    def validate_all(self) -> bool:
        ...
        if self.validation_warnings:
            self.logger.warning(f"Configuration validation has {len(self.validation_warnings)} warnings:")
