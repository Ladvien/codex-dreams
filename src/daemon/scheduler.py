...
class DaemonMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (
            (self.runs_completed / (self.runs_completed + self.runs_failed)) * 100
            if (self.runs_completed + self.runs_failed) > 0
            else 0
        )
        avg_runtime = (
            self.total_runtime / self.runs_completed if self.runs_completed > 0 else timedelta()
        )
...
class DaemonScheduler:
    ...
    def _run_insights_generation(self) -> bool:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                self.logger.info(
                    f"Starting insights generation (attempt {attempt + 1}/{self.config.max_retries + 1})"
                )
                ...
                try:
                    ...
                    self.logger.info(
                        f"Total runs: {self.metrics.runs_completed}, Success rate: {self.metrics.get_stats()['success_rate']}%"
                    )
                    return True
                ...
                self.logger.error(
                    f"❌ Insights generation failed (attempt {attempt + 1}): {error_msg}"
                )
                ...
                if attempt < self.config.max_retries:
                    ...
                    self.logger.error(
                        f"💥 Max retries exceeded, giving up. Total failures: {self.metrics.runs_failed}"
                    )
                    return False
    ...
    def _scheduler_loop(self) -> None:
        ...
        self.logger.info(
            f"⏰ Running insights generation every {self.config.interval_minutes} minutes"
        )
        self.logger.info(
            f"🔄 Max retries: {self.config.max_retries}, Retry delay: {self.config.retry_delay_seconds}s"
        )
        ...
        while self.running:
            ...
            while elapsed < self.config.interval_minutes * 60 and self.running:
                ...
                if (
                    datetime.now() - last_metrics_report
                ).total_seconds() >= metrics_report_interval * 60:
                    self._log_metrics()
                    last_metrics_report = datetime.now()
...
def main() -> None:
    ...
    parser.add_argument("--env-file", type=Path, help="Environment file (.env)")
