...
class ServiceManager:
    ...
    def _install_nssm_service(self, user_mode: bool) -> bool:
        ...
        if self.config.working_directory:
            commands.append(["nssm", "set", service_name, "AppDirectory", self.config.working_directory])
        
        # Add environment file if specified
        if self.config.environment_file:
            commands.append(["nssm", "set", service_name, "AppEnvironment", f"CODEX_ENV_FILE={self.config.environment_file}"])
    ...
    def _install_powershell_service(self, user_mode: bool) -> bool:
        ...
        if result.returncode == 0:
            self.logger.info(f"Successfully installed Windows service '{service_name}' using PowerShell")
            return True
        else:
...
def main():
    ...
    parser.add_argument("--user", action="store_true",
                       help="Install as user service (not system-wide)")
